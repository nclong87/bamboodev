...
class Front_JobController extends Zend_Controller_Action {
	...
	public function viewJobAction() {
		try {
			...
			if (! empty ( $ref )) {
				...
				Core_Utils_DB::query ( 'INSERT DELAYED INTO `refs`(`ref_id`,`ref_type`,`create_time`) VALUES (?,?,NOW())', 3, array (
						$jobId,
						$ref 
				) );
			}
	...
	public function redirectAction() {
		...
		try {
			...
			if (! isset ( $array [$id] ))
				throw new Core_Exception ( 'LINK_ERROR' );
			$this->_forward ( 'view-job', 'job', 'front', array (
					'id' => $array [$id] 
			) );
		} catch ( Exception $e ) {
			$this->view->error_msg = Core_Exception::getErrorMessage ( $e );
	...
	public function editAction() {
		try {
			...
			if ($this->account == null)
				throw new Core_Exception ( 'LOGIN_REQUIRED' );
			if (($job = Application_Model_DbTable_Job::findById ( $jobId )) == null)
				throw new Core_Exception ( 'LINK_ERROR' );
			if (Core_Utils_Tools::isOwner ( $job ) == false)
				throw new Core_Exception ( 'LIMIT_PERMISSION' );
