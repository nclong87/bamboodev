...
class Application_Model_Worker_Register {
	...
	public function start() {
		...
		foreach ( $arrayUsername as $index => $username ) {
			$proxy = '';
			/*
			 * while (!empty($this->_proxy)) { if(!empty($proxy)) break; try { $rand = array_rand($this->_proxy); $proxy = $this->_proxy[$rand]; $cUrl = new Core_Dom_Curl(array( 'method' => 'GET', 'cookie' => $this->cookie, 'proxy' => $proxy )); } catch (Exception $e) { echo 'Remove proxy '.$proxy.PHP_EOL; $proxy = ''; unset($this->_proxy[$rand]); } } if(empty($proxy)) die('No Prxy'); echo 'Use proxy '.$proxy.PHP_EOL;
			 */
