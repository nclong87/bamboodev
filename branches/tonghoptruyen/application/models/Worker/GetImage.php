...
class Application_Model_Worker_GetImage {
	...
	public function start() {
		...
		try {
			...
			Core_Log::getInstance()->log ( 'begin get image' );
			$date = new Zend_Date();
			$date->subHour(24);
			$sql = 'SELECT SQL_CALC_FOUND_ROWS * FROM `chaps` WHERE `status` = 0 AND (last_grab_time IS NULL OR `last_grab_time` <= ?) AND num_fetch_error < 5 ORDER BY `last_grab_time` ASC LIMIT 0,'.$this->limit;
			...
			if(!empty($ids)) {
				$sql = 'UPDATE `chaps` SET `last_grab_time` = now(),num_fetch = num_fetch + 1 WHERE `status` = 0 AND `id` IN ('.join(',', $ids).')';
				$stmt = $db->prepare($sql);
				$stmt->execute();
			}
			foreach ($rows as $row) {
				$rs = Core_Content::getInstance()->getImages($row);
				if($rs != 1) {
					$sql = 'UPDATE `chaps` SET `last_grab_time` = ?,num_fetch_error = num_fetch_error + 1 WHERE `id` = ?';
					$stmt = $db->prepare($sql);
		...
		Core_Log::getInstance()->log ( 'end get image' );
	}
}
