...
class Application_Model_Worker_Poster {
	...
	public function start() {
		...
		// login
		/*
		 * $post_string = 'vb_login_username=chuvandu87&vb_login_password=&x=31&y=16&cookieuser=1&s=9c66b0086628a00af7df6767f1b3ca98&securitytoken=guest&do=login&vb_login_md5password=03e4c83055481daef02e63b77047c826&vb_login_md5password_utf=03e4c83055481daef02e63b77047c826'; //Core_Utils_Log::write($content);die; $cUrl = new Core_Dom_Curl(array( 'method' => 'POST', 'post_fields' => $post_string, 'url' => 'http://www.5giay.vn/login.php?do=login' )); $result = $cUrl->exec(); Core_Utils_Log::write($result['body']);
		 */
		$cUrl = new Core_Dom_Curl ( array (
				'method' => 'GET' 
		) );
		/*
		 * $doc = Core_Dom_Query::newDocumentHTML($content,'UTF-8'); $post_data = $doc->find('#login_form')->serializeArray(); $post_items = array(); $array = array( 'email' => 'test@jobbid.vn', 'pass' => '74198788' ); foreach ($post_data as $item) { if(isset($array[$item['name']])) $item['value'] = $array[$item['name']]; $post_items[] = $item['name'] . '=' . $item['value']; } $post_string = implode ('&', $post_items); $cUrl = new Core_Dom_Curl(array( 'method' => 'POST', 'post_fields' => $post_string, 'url' => 'https://www.facebook.com/login.php?login_attempt=1', )); $result = $cUrl->exec();
		 */
		// $content = $cUrl->getContent('https://www.facebook.com/find-friends/browser/?rpix=1');
		/*
		 * $doc = Core_Dom_Query::newDocumentHTML($content,'UTF-8'); foreach ($doc['.friendBrowserListUnit'] as $item) { $item = pq($item); $uid = $item->find('.friendBrowserID')->val(); echo 'Add friend '.$uid.PHP_EOL; $cUrl = new Core_Dom_Curl(array( 'method' => 'POST', 'post_fields' => 'to_friend='.$uid.'&action=add_friend&how_found=friend_browser&ref_param=none&&&outgoing_id=&logging_location=friend_browser&no_flyout_on_click=true&ego_log_data&http_referer&__user=100005325785808&__a=1&__dyn=7n8aD5z5zu&__req=g&fb_dtsg=AQB8OFrV&phstamp=165816656797011486251', 'url' => 'https://www.facebook.com/ajax/add_friend/action.php', )); $result = $cUrl->exec(); echo $result['body'].PHP_EOL; }
		 */
