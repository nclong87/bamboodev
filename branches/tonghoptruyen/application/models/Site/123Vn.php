...
class Application_Model_Site_123Vn {
	...
	public function login($zingid, $flag = 1) {
		if ($flag == 1) {
			$length = count ( $zingid );
	...
	public function guest($price) {
		...
		$cUrl = new Core_Dom_Curl ( array (
				...
				'header' => array (
						...
						'Host: 123.vn',
						'Referer: http://123.vn/dau-gia-nguoc.html',
	...
	public function start() {
		...
		$logMsg = $msg;
		echo $msg;
		$this->login ( $row ['zingid'], 2 );
		// $num = Core_Utils_DB::query('SELECT MAX(number) as num FROM `bbd_guest` WHERE zingid IS NOT NULL',2);
		$rows = Core_Utils_DB::query ( 'SELECT * FROM `bbd_guest` WHERE `zingid` IS NULL ORDER BY RAND() LIMIT 0,5' );
		// $i = rand(0, 3);
		// echo $i.PHP_EOL;
		foreach ( $rows as $item ) {
			...
			if ($result ['bid'] == true) {
				echo ' => OK' . PHP_EOL;
				Core_Utils_DB::update ( 'bbd_guest', array (
						'zingid' => $row ['zingid'],
						'time_create' => Core_Utils_Date::getCurrentDateSQL () 
				), array (
						'number' => $item ['number'] 
				) );
			} else {
		...
		Core_Utils_DB::update ( 'zing', array (
				'status' => '0' 
		), array (
				'id' => $row ['id'] 
		) );
	...
	public function test($num) {
		...
		if (empty ( $this->cookie )) {
			...
			$zingid = $row ['zingid'];
			$this->login ( $zingid, 2 );
			echo 'Guest zingid ' . $zingid . PHP_EOL;
		}
		if (is_array ( $num )) {
			foreach ( $num as $item ) {
				echo PHP_EOL;
				echo 'Guest num ' . $item . ' => ';
				$result = $this->guest ( $item . '000' );
				if ($result ['bid'] == true) {
					Core_Utils_DB::update ( 'bbd_guest', array (
							'zingid' => $zingid,
							'time_create' => Core_Utils_Date::getCurrentDateSQL () 
	...
	public function getGuest($zingid) {
		...
		return $cUrl->getContent ( 'http://123.vn/auction/historybid?product_id=71342' );
	}
}
