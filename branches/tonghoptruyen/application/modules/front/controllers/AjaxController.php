...
class Front_AjaxController extends Zend_Controller_Action {
	...
	public function refeshCaptchaAction() {
		...
		if (! in_array ( $type, Core_Const::$captcha_type ))
			die ( 'ERROR' );
	...
	public function checkCaptchaAction() {
		...
		if (! in_array ( $type, Core_Const::$captcha_type ))
			die ( 'ERROR' );
		$session = new Zend_Session_Namespace ( 'front' );
		if (! isset ( $session->captcha_word [$type] ))
			die ( 'ERROR' );
	...
	public function verifyRegistrationAction() {
		...
		Core_Utils_DB::update ( 'accounts', array (
				'active' => 1 
		), array (
				'id' => $secure_key ['account_id'],
				'status' => 1 
		) );
		if (($account = Application_Model_DbTable_TaiKhoan::findbyId ( $secure_key ['account_id'] )) == null)
			die ( 'ERROR' );
	...
	public function viewJobAction() {
		...
		if (! isset ( $this->session->token ) || empty ( $token ) || $this->session->token != $token)
			die ();
		$jobId = $this->_request->getParam ( 'id', '' );
		if (! empty ( $jobId )) {
			Core_Utils_DB::query ( 'UPDATE `jobs` SET `view` = `view` + 1 WHERE `id` = ?', 3, array (
					$jobId 
			) );
	...
	public function verifyJobAction() {
		...
		Application_Model_DbTable_SecureKey::removeSecureKey ( $secure_key ['id'] );
		Core_Utils_DB::update ( 'jobs', array (
				'active' => 1 
		), array (
				'id' => $secure_key ['ref_id'],
				'status' => 1 
		) );
		die ( 'OK' );
	}
	public function reportJobAction() {
		...
		if (empty ( $jobId ))
			die ( 'ERROR' );
		if (isset ( $this->session->reports [$jobId] ))
			die ( 'OK' );
...
- Nội dung công việc, thời gian làm việc và các vấn đề chi tiết sẽ trao đổi trong vòng phỏng vấn';
		...
		// echo $key;
		/*
		 * $str = file_get_contents('https://graph.facebook.com/me?access_token=AAACF3doGSoEBAD0ph0PHsAftEr5GVbZBR0IjpximstoVGjiwj2xbSuVxVXKIO3qlS5VyJ0ZCA9zsa7L2GMMZB1dhq0HgiIsrZCCjm7uP0XtuA3jOkVcy'); $array = json_decode($str); print_r($array);
		 */
	...
	/*
	 * public function sendOrderMailAction() { $purchase_id = $this->_request->getParam('id',''); if(!empty($purchase_id)) { $dbPurchase = new Application_Model_DbTable_Purchase(); if(($purchase = $dbPurchase->findById($purchase_id)) != null) { if($purchase->is_send == false) { $dbGianhang = new Application_Model_DbTable_GianHang(); if(($gianhang = $dbGianhang->findById($purchase->gianhang_id)) == null) return; $link = DOMAIN.$this->getFrontController()->getBaseUrl().'/'.$gianhang->ma_gian_hang.'/phieu-dat-hang?id='.$purchase->id.'&code='.$purchase->code; $link = '<a href="'.$link.'">'.$link.'</a>'; $send_mail = new zendcms_Controller_Helper_SendMail(); $send_mail->sendEmailByTemplate($gianhang->email_gianhang, array( 'link' => $link ),TEMPLATE_ORDER); $dbPurchase->update(array('is_send' => 1), array('id = ?' => $purchase_id)); } } } }
	 */
}
