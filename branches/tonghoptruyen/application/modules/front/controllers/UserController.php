...
class Front_UserController extends Zend_Controller_Action {
	...
	public function updateAction() {
		try {
			...
			if ($this->getRequest ()->isPost ()) {
				$form_data = $this->getRequest ()->getParams ();
				$form_data = Core_Utils_Tools::strip_tags ( $form_data, array (
						'password' 
				) );
				if ($form->isValid ( $form_data )) {
					$data_update = array (
					...
					Core_Utils_DB::update ( 'accounts', $data_update, array (
							'id' => $this->session->logged ['id'] 
					) );
		...
		try {
			...
			if ($this->getRequest ()->isPost ()) {
				...
				if (empty ( $new_password )) {
					...
					Core_Utils_DB::update ( 'secure_keys', array (
							'status' => 0 
					), array (
							'id' => $secure_key ['id'] 
					) );
