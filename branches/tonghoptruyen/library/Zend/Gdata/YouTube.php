...
class Zend_Gdata_YouTube extends Zend_Gdata_Media {
	...
	const STANDARD_TOP_RATED_URI_V2 = 'http://gdata.youtube.com/feeds/api/standardfeeds/top_rated';
	const STANDARD_MOST_VIEWED_URI_V2 = 'http://gdata.youtube.com/feeds/api/standardfeeds/most_viewed';
	const STANDARD_RECENTLY_FEATURED_URI_V2 = 'http://gdata.youtube.com/feeds/api/standardfeeds/recently_featured';
	const STANDARD_WATCH_ON_MOBILE_URI_V2 = 'http://gdata.youtube.com/feeds/api/standardfeeds/watch_on_mobile';
	...
	const FRIEND_ACTIVITY_FEED_URI = 'http://gdata.youtube.com/feeds/api/users/default/friendsactivity';
	 ...
	 * @var string
	 */
	const IN_REPLY_TO_SCHEME = 'http://gdata.youtube.com/schemas/2007#in-reply-to';
	 ...
	 * @var string
	 */
	const INBOX_FEED_URI = 'http://gdata.youtube.com/feeds/api/users/default/inbox';
	 ...
	 *         specified URL.
	 */
	public function getVideoEntry($videoId = null, $location = null, $fullEntry = false) {
		if ($videoId !== null) {
			if ($fullEntry) {
	 ...
	 *        	The ID of the video to retrieve.
	 * @throws Zend_Gdata_App_HttpException
	 * @return Zend_Gdata_YouTube_VideoEntry null video entry to be
	...
	public function getFullVideoEntry($videoId) {
		$uri = self::USER_URI . "/default/" . self::UPLOADS_URI_SUFFIX . "/$videoId";
		return parent::getEntry ( $uri, 'Zend_Gdata_YouTube_VideoEntry' );
	}
	...
	public function getUserUploads($user = null, $location = null) {
		if ($user !== null) {
			$uri = self::USER_URI . '/' . $user . '/' . self::UPLOADS_URI_SUFFIX;
		} else if ($location instanceof Zend_Gdata_Query) {
			$uri = $location->getQueryUrl ();
		} else {
			$uri = $location;
	...
	public function getUserFavorites($user = null, $location = null) {
		if ($user !== null) {
			$uri = self::USER_URI . '/' . $user . '/' . self::FAVORITES_URI_SUFFIX;
		} else if ($location instanceof Zend_Gdata_Query) {
			$uri = $location->getQueryUrl ();
		} else {
			$uri = $location;
	 ...
	 *        	A string identifying the usernames for which to
	...
	public function getActivityForUser($username) {
		if ($this->getMajorProtocolVersion () == 1) {
			...
			throw new Zend_Gdata_App_VersionException ( 'User activity feeds ' . 'are not available in API version 1.' );
		}
		...
		if ($username instanceof Zend_Gdata_Query) {
			...
			if (count ( explode ( ',', $username ) ) > self::ACTIVITY_FEED_MAX_USERS) {
				require_once 'Zend/Gdata/App/InvalidArgumentException.php';
				throw new Zend_Gdata_App_InvalidArgumentException ( 'Activity feed can only retrieve for activity for up to ' . self::ACTIVITY_FEED_MAX_USERS . ' users per request' );
			}
	 ...
	 * @return Zend_Gdata_YouTube_InboxEntry null Zend_Gdata_YouTube_Inbox_Entry representing the sent message.
	 *        
	 */
	public function sendVideoMessage($body, $videoEntry = null, $videoId = null, $recipientUserName) {
		if (! $videoId && ! $videoEntry) {
			require_once 'Zend/Gdata/App/InvalidArgumentException.php';
			throw new Zend_Gdata_App_InvalidArgumentException ( 'Expecting either a valid videoID or a videoEntry object in ' . 'Zend_Gdata_YouTube->sendVideoMessage().' );
		}
		...
		if ($this->getMajorProtocolVersion () == null || $this->getMajorProtocolVersion () == 1) {
			...
			$messageEntry->setDescription ( new Zend_Gdata_YouTube_Extension_Description ( $body ) );
		} else {
			if (! $videoId) {
				$videoId = $videoEntry->getVideoId ();
		...
		$insertUrl = 'http://gdata.youtube.com/feeds/api/users/' . $recipientUserName . '/inbox';
		$response = $this->insertEntry ( $messageEntry, $insertUrl, 'Zend_Gdata_YouTube_InboxEntry' );
		return $response;
	}
	 ...
	 *
	 * @param Zend_Gdata_YouTube_CommentEntry $commentEntry
	 *        	The comment entry
	 *        	to reply to
