...
class Zend_Gdata_Health extends Zend_Gdata {
	...
	/**
	 * Profile ID used for all API interactions.
	 ...
	 *        	The identity of the application in the form
	 *        	of Company-AppName-Version
	 * @param bool $useH9Sandbox
	 *        	True if the H9 Developer's Sandbox should be
	...
	public function getHealthProfileListFeed($query = null) {
		if ($this->_httpClient->getClientLoginToken () === null) {
			...
			throw new Zend_Gdata_App_AuthException ( 'Profiles list feed is only available when using ClientLogin' );
		}
	...
	/**
	 * Retrieve a user's profile as a feed object.
	 * If ClientLogin is used, the
	...
	public function getHealthProfileFeed($query = null) {
		...
		if ($query instanceof Zend_Gdata_Query) {
			$uri = $query->getQueryUrl ();
		} else if ($this->_httpClient->getClientLoginToken () !== null && $query == null) {
	...
	/**
	 * Posts a new notice using the register feed.
	 ...
	 *        	The (optional) type of message body
	 *        	(text, xhtml, html, etc.)
	 * @param string $ccrXML
