...
class Zend_Feed_Writer_Renderer_Entry_Rss extends Zend_Feed_Writer_Renderer_RendererAbstract implements Zend_Feed_Writer_Renderer_RendererInterface {
	...
	protected function _setTitle(DOMDocument $dom, DOMElement $root) {
		if (! $this->getDataContainer ()->getDescription () && ! $this->getDataContainer ()->getTitle ()) {
	...
	protected function _setDateModified(DOMDocument $dom, DOMElement $root) {
		...
		$text = $dom->createTextNode ( $this->getDataContainer ()->getDateModified ()->get ( Zend_Date::RSS ) );
	...
	protected function _setDateCreated(DOMDocument $dom, DOMElement $root) {
		...
		if (! $this->getDataContainer ()->getDateModified ()) {
			$this->getDataContainer ()->setDateModified ( $this->getDataContainer ()->getDateCreated () );
		}
	}
	...
	protected function _setEnclosure(DOMDocument $dom, DOMElement $root) {
		...
		if (isset ( $data ['length'] ) && ( int ) $data ['length'] <= 0) {
			require_once 'Zend/Feed/Exception.php';
			$exception = new Zend_Feed_Exception ( 'Enclosure "length" must be an integer' . ' indicating the content\'s length in bytes' );
			if (! $this->_ignoreExceptions) {
				throw $exception;
			} else {
				$this->_exceptions [] = $exception;
