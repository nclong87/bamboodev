...
class Zend_Feed_Reader_Extension_Atom_Entry extends Zend_Feed_Reader_Extension_EntryAbstract {
	...
	public function getBaseUrl() {
		...
		$baseUrl = $this->getXpath ()->evaluate ( 'string(' . $this->getXpathPrefix () . '/@xml:base[1]' . ')' );
	...
	public function getLinks() {
		...
		$list = $this->getXpath ()->query ( $this->getXpathPrefix () . '//atom:link[@rel="alternate"]/@href' . '|' . $this->getXpathPrefix () . '//atom:link[not(@rel)]/@href' );
	...
	public function getCommentCount() {
		...
		$list = $this->getXpath ()->query ( $this->getXpathPrefix () . '//atom:link[@rel="replies"]/@thread10:count' );
	...
	public function getCommentLink() {
		...
		$list = $this->getXpath ()->query ( $this->getXpathPrefix () . '//atom:link[@rel="replies" and @type="text/html"]/@href' );
	...
	public function getCommentFeedLink($type = 'atom') {
		...
		$list = $this->getXpath ()->query ( $this->getXpathPrefix () . '//atom:link[@rel="replies" and @type="application/' . $type . '+xml"]/@href' );
	...
	public function getCategories() {
		...
		if ($this->_getAtomType () == Zend_Feed_Reader::TYPE_ATOM_10) {
			 ...
			 * However there is a small possibility Atom 0.3
