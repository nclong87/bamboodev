...
class Zend_OpenId_Extension_Sreg extends Zend_OpenId_Extension {
	/**
	 * SREG 1.1 namespace.
	 * All OpenID SREG 1.1 messages MUST contain variable
	...
	public function parseRequest($params) {
		if (isset ( $params ['openid_ns_sreg'] ) && $params ['openid_ns_sreg'] === Zend_OpenId_Extension_Sreg::NAMESPACE_1_1) {
			$this->_version = 1.1;
		} else {
			$this->_version = 1.0;
		}
	...
	public function parseResponse($params) {
		...
		if (isset ( $this->_props ) && is_array ( $this->_props )) {
			foreach ( self::getSregProperties () as $prop ) {
				if (isset ( $this->_props [$prop] ) && $this->_props [$prop] && ! isset ( $props [$prop] )) {
					return false;
				}
			}
		}
	 ...
	 * OpenId request.
	 * On success sets SREG response properties from given
