...
class Zend_OpenId_Provider {
	 ...
	 * in user.
	 * Next time this consumer will try to authenticate the user,
	 ...
	 *
	 * @param array $params
	 *        	GET or POST variables. If this parameter is omited
	...
	protected function _associate($version, $params) {
		...
		if (isset ( $params ['openid_assoc_type'] ) && $params ['openid_assoc_type'] == 'HMAC-SHA1') {
			$macFunc = 'sha1';
		} else if (isset ( $params ['openid_assoc_type'] ) && $params ['openid_assoc_type'] == 'HMAC-SHA256' && $version >= 2.0) {
			$macFunc = 'sha256';
		} else {
		...
		} else if (isset ( $params ['openid_session_type'] ) && $params ['openid_session_type'] == 'DH-SHA1') {
			$dhFunc = 'sha1';
		} else if (isset ( $params ['openid_session_type'] ) && $params ['openid_session_type'] == 'DH-SHA256' && $version >= 2.0) {
			$dhFunc = 'sha256';
		} else {
	...
	protected function _checkId($version, $params, $immediate, $extensions = null, Zend_Controller_Response_Abstract $response = null) {
		...
		if ($trusted === false) {
			...
			if ($immediate) {
				$params2 ['openid.mode'] = 'checkid_setup';
				$ret ['openid.mode'] = ($version >= 2.0) ? 'setup_needed' : 'id_res';
				$ret ['openid.user_setup_url'] = $this->_trustUrl . (strpos ( $this->_trustUrl, '?' ) === false ? '?' : '&') . Zend_OpenId::paramsToQuery ( $params2 );
				return $ret;
			} else {
