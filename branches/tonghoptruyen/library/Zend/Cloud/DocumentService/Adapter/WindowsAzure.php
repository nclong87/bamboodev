...
class Zend_Cloud_DocumentService_Adapter_WindowsAzure extends Zend_Cloud_DocumentService_Adapter_AbstractAdapter {
	...
	public function __construct($options = array()) {
		...
		try {
			$this->_storageClient = new Zend_Service_WindowsAzure_Storage_Table ( $host, $options [self::ACCOUNT_NAME], $options [self::ACCOUNT_KEY] );
			// Parse other options
	 ...
	 *
	 * @param string $collectionName        	
	 * @param mixed|Zend_Cloud_DocumentService_Document $documentId
	...
	public function fetchDocument($collectionName, $documentId, $options = null) {
		...
		try {
			...
			$documentClass = $this->getDocumentClass ();
			return new $documentClass ( $this->_resolveAttributes ( $entity ), array (
	...
	public function query($collectionName, $query, $options = null) {
		try {
			...
			foreach ( $entities as $entity ) {
				$resultSet [] = new $documentClass ( $this->_resolveAttributes ( $entity ), array (
						$entity->getPartitionKey (),
						$entity->getRowKey () 
				) );
			}
