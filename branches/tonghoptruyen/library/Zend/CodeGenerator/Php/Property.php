...
class Zend_CodeGenerator_Php_Property extends Zend_CodeGenerator_Php_Member_Abstract {
	...
	public function generate() {
		...
		if ($this->isConst ()) {
			...
			$output .= $this->_indentation . 'const ' . $name . ' = ' . (($defaultValue !== null) ? $defaultValue->generate () : 'null;');
		} else {
			$output .= $this->_indentation . $this->getVisibility () . (($this->isStatic ()) ? ' static' : '') . ' $' . $name . ' = ' . (($defaultValue !== null) ? $defaultValue->generate () : 'null;');
		}
		return $output;
	}
}
