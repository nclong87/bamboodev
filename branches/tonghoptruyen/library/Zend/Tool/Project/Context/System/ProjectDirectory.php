...
class Zend_Tool_Project_Context_System_ProjectDirectory extends Zend_Tool_Project_Context_Filesystem_Directory implements Zend_Tool_Project_Context_System_Interface, Zend_Tool_Project_Context_System_NotOverwritable, Zend_Tool_Project_Context_System_TopLevelRestrictable {
	...
	public function create() {
		if (file_exists ( $this->getPath () )) {
			/*
			 * foreach (new DirectoryIterator($this->getPath()) as $item) { if (!$item->isDot()) { if ($registry->getClient()->isInteractive()) { // @todo prompt for override } else { require_once 'Zend/Tool/Project/Context/Exception.php'; throw new Zend_Tool_Project_Context_Exception('This directory is not empty, project creation aborted.'); } break; } }
			 */
		}
