...
class Zend_Tool_Project_Provider_Action extends Zend_Tool_Project_Provider_Abstract implements Zend_Tool_Framework_Provider_Pretendable {
	...
	public static function createResource(Zend_Tool_Project_Profile $profile, $actionName, $controllerName, $moduleName = null) {
		...
		$actionMethod = $controllerFile->createResource ( 'ActionMethod', array (
	...
	public static function hasResource(Zend_Tool_Project_Profile $profile, $actionName, $controllerName, $moduleName = null) {
		...
		return (($controllerFile->search ( array (
				'actionMethod' => array (
						'actionName' => $actionName 
				) 
	...
	protected static function _getControllerFileResource(Zend_Tool_Project_Profile $profile, $controllerName, $moduleName = null) {
		...
		$profileSearchParams ['controllerFile'] = array (
	 ...
	 *
	 * @param string $name
	 *        	Action name for controller, in camelCase format.
	 * @param string $controllerName
	 *        	Controller name action should be applied to.
	 * @param bool $viewIncluded
	 *        	Whether the view should the view be included.
	 * @param string $module
	 *        	Module name action should be applied to.
	 */
	public function create($name, $controllerName = 'Index', $viewIncluded = true, $module = null) {
		...
		if ($testingEnabled && ! Zend_Tool_Project_Provider_Test::isPHPUnitAvailable ()) {
			...
			$response->appendContent ( 'Note: PHPUnit is required in order to generate controller test stubs.', array (
					'color' => array (
							'yellow' 
					) 
			) );
		...
		if ($name !== $originalName) {
			$response->appendContent ( 'Note: The canonical action name that ' . $tense . ' used with other providers is "' . $name . '";' . ' not "' . $originalName . '" as supplied', array (
					'color' => array (
							'yellow' 
					) 
			) );
		...
		if ($controllerName !== $originalControllerName) {
			$response->appendContent ( 'Note: The canonical controller name that ' . $tense . ' used with other providers is "' . $controllerName . '";' . ' not "' . $originalControllerName . '" as supplied', array (
					'color' => array (
							'yellow' 
					) 
			) );
