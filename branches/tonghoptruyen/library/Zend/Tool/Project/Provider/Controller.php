...
class Zend_Tool_Project_Provider_Controller extends Zend_Tool_Project_Provider_Abstract implements Zend_Tool_Framework_Provider_Pretendable {
	 ...
	 * profile.
	 * NOTE: it is your job to execute the create() method on the resource, as well as
	...
	public static function hasResource(Zend_Tool_Project_Profile $profile, $controllerName, $moduleName = null) {
		...
		return ($controllersDirectory && ($controllersDirectory->search ( array (
	 ...
	 *
	 * @param string $name
	 *        	The name of the controller to create, in camelCase.
	 * @param bool $indexActionIncluded
	 *        	Whether or not to create the index action.
	 */
	public function create($name, $indexActionIncluded = true, $module = null) {
		...
		if (($name !== $originalName)) {
			...
			$response->appendContent ( 'Note: The canonical controller name that ' . $tense . ' used with other providers is "' . $name . '";' . ' not "' . $originalName . '" as supplied', array (
					'color' => array (
							'yellow' 
					) 
			) );
