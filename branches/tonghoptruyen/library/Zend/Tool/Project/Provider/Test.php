...
class Zend_Tool_Project_Provider_Test extends Zend_Tool_Project_Provider_Abstract {
	...
	public static function createApplicationResource(Zend_Tool_Project_Profile $profile, $controllerName, $actionName, $moduleName = null) {
		...
		if ($moduleName) {
			...
			if (($testAppModuleDirectoryResource = $testAppModulesDirectoryResource->search ( array (
					'testApplicationModuleDirectory' => array (
							'forModuleName' => $moduleName 
					) 
			) )) === false) {
				$testAppModuleDirectoryResource = $testAppModulesDirectoryResource->createResource ( 'testApplicationModuleDirectory', array (
						'forModuleName' => $moduleName 
				) );
			}
		...
		if (($testAppControllerFileResource = $testAppControllerDirectoryResource->search ( array (
				'testApplicationControllerFile' => array (
		...
		return $testAppControllerFileResource->createResource ( 'testApplicationActionMethod', array (
				'forActionName' => $actionName 
		) );
	}
	...
	public static function createLibraryResource(Zend_Tool_Project_Profile $profile, $libraryClassName) {
		$testLibraryDirectoryResource = $profile->search ( array (
				'TestsDirectory',
		...
		while ( $nameOrNamespacePart = array_shift ( $fsParts ) ) {
			...
			if (count ( $fsParts ) > 0) {
				...
				if (($libraryDirectoryResource = $currentDirectoryResource->search ( array (
						'TestLibraryNamespaceDirectory' => array (
				...
				if (($libraryFileResource = $currentDirectoryResource->search ( array (
						'TestLibraryFile' => array (
								'forClassName' => $libraryClassName 
						) 
				) )) === false) {
					$libraryFileResource = $currentDirectoryResource->createResource ( 'TestLibraryFile', array (
							'forClassName' => $libraryClassName 
					) );
				}
			}
