 ...
 * @version    $Id: ProjectProvider.php 23775 2011-03-01 17:25:24Z ralph $
 */
...
class Zend_Tool_Project_Provider_ProjectProvider extends Zend_Tool_Project_Provider_Abstract {
	...
	public static function createResource(Zend_Tool_Project_Profile $profile, $projectProviderName, $actionNames = null) {
		...
		$projectProvider = $profile->createResourceAt ( $profileSearchParams, 'projectProviderFile', array (
				'projectProviderName' => $projectProviderName,
	...
	public function create($name, $actions = null) {
		...
		if ($this->_registry->getRequest ()->isPretend ()) {
			$this->_registry->getResponse ()->appendContent ( 'Would create a project provider named ' . $name . ' in location ' . $projectProvider->getPath () );
		} else {
			$this->_registry->getResponse ()->appendContent ( 'Creating a project provider named ' . $name . ' in location ' . $projectProvider->getPath () );
