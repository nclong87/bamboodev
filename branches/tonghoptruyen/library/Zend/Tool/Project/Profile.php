 ...
 * @version    $Id: Profile.php 23775 2011-03-01 17:25:24Z ralph $
 */
...
class Zend_Tool_Project_Profile extends Zend_Tool_Project_Profile_Resource_Container {
	...
	public function getIterator() {
		...
		return new RecursiveIteratorIterator ( new Zend_Tool_Project_Profile_Iterator_EnabledResourceFilter ( $this ), RecursiveIteratorIterator::SELF_FIRST );
	}
	...
	public function __toString() {
		...
		foreach ( $this as $resource ) {
			...
			foreach ( $rii as $item ) {
				$string .= str_repeat ( '  ', $rii->getDepth () + 1 ) . $item->getName () . ((count ( $attributes = $item->getAttributes () ) > 0) ? ' [' . http_build_query ( $attributes ) . ']' : '') . PHP_EOL;
			}
		}
		return $string;
	}
