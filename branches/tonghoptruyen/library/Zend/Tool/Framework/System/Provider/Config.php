...
class Zend_Tool_Framework_System_Provider_Config extends Zend_Tool_Framework_Provider_Abstract {
	...
	public function show() {
		...
		$resp->appendContent ( "User Configuration: " . $userConfig->getConfigFilepath (), array (
				"color" => "green" 
		) );
		$resp->appendContent ( $tree, array (
				"indention" => 2 
		) );
	}
	...
	protected function _doEnable($className) {
		...
		if (! in_array ( $className, $providerClasses )) {
			...
			if ($userConfig->save ()) {
				$this->_registry->getResponse ()->appendContent ( "Provider/Manifest '" . $className . "' was enabled for usage with Zend Tool.", array (
						"color" => "green",
						"aligncenter" => true 
				) );
			} else {
				require_once "Zend/Tool/Framework/Exception.php";
				throw new Zend_Tool_Framework_Exception ( "Could not write user configuration to persistence." );
			}
		} else {
			require_once "Zend/Tool/Framework/Exception.php";
			throw new Zend_Tool_Framework_Exception ( "Provider/Manifest '" . $className . "' is already enabled." );
		}
	}
	...
	public function disableProvider($className) {
		...
		if (($key = array_search ( $className, $providerClasses )) !== false) {
			...
			if ($userConfig->save ()) {
				$this->_registry->getResponse ()->appendContent ( "Provider/Manifest '" . $className . "' was disabled.", array (
						"color" => "green",
						"aligncenter" => true 
				) );
			} else {
			...
			require_once "Zend/Tool/Framework/Exception.php";
			throw new Zend_Tool_Framework_Exception ( "Provider/Manifest '" . $className . "' is not enabled." );
		}
	}
	...
	protected function _loadUserConfigIfExists() {
		...
		$resp = $this->_registry->getResponse ();
		if (! $userConfig->exists ()) {
			$resp->appendContent ( "User has no config file.", array (
					"aligncenter" => true,
					"color" => array (
							'hiWhite',
							'bgRed' 
