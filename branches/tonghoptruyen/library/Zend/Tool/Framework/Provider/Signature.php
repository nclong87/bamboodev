 ...
 * @version    $Id: Signature.php 23775 2011-03-01 17:25:24Z ralph $
 */
...
class Zend_Tool_Framework_Provider_Signature implements Zend_Tool_Framework_Registry_EnabledInterface {
	...
	public function getActionableMethodByActionName($actionName, $specialtyName = '_Global') {
		foreach ( $this->_actionableMethods as $actionableMethod ) {
			if ($actionName == $actionableMethod ['actionName'] && $specialtyName == $actionableMethod ['specialty']) {
	...
	protected function _processSpecialties() {
		...
		if ($this->_providerReflection->hasMethod ( 'getSpecialties' )) {
			...
			if (! is_array ( $specialties )) {
				...
				throw new Zend_Tool_Framework_Provider_Exception ( 'Provider ' . get_class ( $this->_provider ) . ' must return an array for method getSpecialties().' );
			}
		} else {
			...
			if (! is_array ( $specialties )) {
				require_once 'Zend/Tool/Framework/Provider/Exception.php';
				throw new Zend_Tool_Framework_Provider_Exception ( 'Provider ' . get_class ( $this->_provider ) . '\'s property $_specialties must be an array.' );
			}
		}
		
		$this->_specialties = array_merge ( array (
				'_Global' 
		), $specialties );
	}
	...
	protected function _processActionableMethods() {
		...
		foreach ( $methods as $method ) {
			 ...
			 * contain the name "
			 */
			if (! $method->getDeclaringClass ()->isInstantiable () || ! $method->isPublic () || $methodName [0] == '_' || $method->isStatic () || in_array ( $methodName, array (
					'getContextClasses',
					'getName' 
			) )) 			// other protected public methods will nee to go here
                ) {// other protected public methods will nee to go here
			null
				continue;
			}
            ...
            $action// get the action, and create non-existent actions when they dont exist (the true part below)
			null = $this->_registry->getActionRepository ()->getAction ( $actionableMethods [$methodName] ['actionName'] );
			if ($action == null) {
