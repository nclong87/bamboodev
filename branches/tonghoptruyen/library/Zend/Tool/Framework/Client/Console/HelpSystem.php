...
class Zend_Tool_Framework_Client_Console_HelpSystem {
	...
	public function respondWithErrorMessage($errorMessage, Exception $exception = null) {
		...
		$this->_response->appendContent ( $text, array (
				'color' => array (
						'hiWhite',
						'bgRed' 
				),
		...
		$this->_response->appendContent ( $errorMessage, array (
				'indention' => 1,
				'blockize' => 72,
				'color' => array (
						'white',
		...
		$this->_response->appendContent ( null, array (
				'separator' => true 
		) );
		return $this;
	}
	...
	public function respondWithGeneralHelp() {
		...
		$this->_response->appendContent ( 'Usage:', array (
				'color' => 'green' 
		) )->appendContent ( '    ', $noSeparator )->appendContent ( 'zf', array_merge ( array (
				'color' => 'cyan' 
		), $noSeparator ) )->appendContent ( ' [--global-opts]', $noSeparator )->appendContent ( ' action-name', array_merge ( array (
				'color' => 'cyan' 
		), $noSeparator ) )->appendContent ( ' [--action-opts]', $noSeparator )->appendContent ( ' provider-name', array_merge ( array (
				'color' => 'cyan' 
		), $noSeparator ) )->appendContent ( ' [--provider-opts]', $noSeparator )->appendContent ( ' [provider parameters ...]' )->appendContent ( '    Note: You may use "?" in any place of the above usage string to ask for more specific help information.', array (
				'color' => 'yellow' 
		) )->appendContent ( '    Example: "zf ? version" will list all available actions for the version provider.', array (
				'color' => 'yellow',
				'separator' => 2 
		) )->appendContent ( 'Providers and their actions:', array (
				'color' => 'green' 
		) );
	...
	public function respondWithActionHelp($actionName) {
		$this->_respondWithHeader ();
		$this->_response->appendContent ( 'Providers that support the action "' . $actionName . '"', array (
				'color' => 'green' 
		) );
	...
	public function respondWithSpecialtyAndParamHelp($providerName, $actionName) {
		$this->_respondWithHeader ();
		$this->_response->appendContent ( 'Details for action "' . $actionName . '" and provider "' . $providerName . '"', array (
				'color' => 'green' 
		) );
	...
	public function respondWithProviderHelp($providerName) {
		$this->_respondWithHeader ();
		$this->_response->appendContent ( 'Actions supported by provider "' . $providerName . '"', array (
				'color' => 'green' 
		) );
	...
	protected function _respondWithHeader() {
		...
		$this->_response->appendContent ( 'Zend Framework', array (
				'color' => array (
						'hiWhite' 
				),
				'separator' => false 
	...
	protected function _respondWithSystemInformation($providerNameFilter = null, $actionNameFilter = null, $includeAllSpecialties = false) {
		...
		if (is_string ( $providerNameFilter )) {
			$providerMetadatasSearch = array_merge ( $providerMetadatasSearch, array (
		...
		if (is_string ( $actionNameFilter )) {
			$actionMetadatasSearch = array_merge ( $actionMetadatasSearch, array (
		...
		foreach ( $displayProviderMetadatas as $providerMetadata ) {
			...
			foreach ( $providerSignature->getActions () as $actionInfo ) {
				...
				if (! $includeAllSpecialties && count ( $actionableGlobalMetadatas ) > 1) {
					$this->_response->appendContent ( '    Note: There are specialties, use ', array (
							'color' => 'yellow',
							'separator' => false 
					) );
					$this->_response->appendContent ( 'zf ' . $actionMetadata->getValue () . ' ' . $providerMetadata->getValue () . '.?', array (
							'color' => 'cyan',
							'separator' => false 
					) );
					$this->_response->appendContent ( ' to get specific help on them.', array (
							'color' => 'yellow' 
					) );
				}
			}
	                                                          ...
	                                                          // Zend_Tool_Framework_Metadata_Tool $parameterShortMetadata)
	{
		...
		if ($specialtyMetadata->getSpecialtyName () != '_Global') {
			$this->_response->appendContent ( '.' . $specialtyMetadata->getValue (), array (
					'color' => 'cyan',
					'separator' => false 
			) );
		}
		...
		foreach ( $parameterLongMetadata->getValue () as $paramName => $consoleParamName ) {
			...
			$this->_response->appendContent ( $paramString . '', array (
					'separator' => false 
			) );
		}
