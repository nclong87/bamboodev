 ...
 * @version    $Id: ArgumentParser.php 23775 2011-03-01 17:25:24Z ralph $
 */
...
class Zend_Tool_Framework_Client_Console_ArgumentParser implements Zend_Tool_Framework_Registry_EnabledInterface {
	...
	public function parse() {
		...
		if (count ( $this->_argumentsWorking ) == 0) {
			...
			if ($this->_help) {
				...
				$this->_createHelpResponse ( array (
						'error' => 'An action and provider is required.' 
				) );
			}
			return;
		}
		...
		if ($this->_helpKnownAction) {
			$helpResponseOptions = array_merge ( $helpResponseOptions, array (
		...
		if (count ( $this->_argumentsWorking ) == 0) {
			...
			if ($this->_help) {
				...
				$this->_createHelpResponse ( array_merge ( $helpResponseOptions, array (
	...
	protected function _parseGlobalPart() {
		...
		$getoptParser = new Zend_Console_Getopt ( $getoptOptions, $this->_argumentsWorking, array (
				'parseAll' => false 
		) );
	...
	protected function _parseActionPart() {
		...
		if (! $actionMetadata) {
			...
			$actionSearchCriteria ['value'] = strtolower ( str_replace ( array (
					'-',
					'_' 
			), '', $consoleActionName ) );
	...
	protected function _parseProviderPart() {
		...
		if (! $providerMetadata) {
			...
			$providerSearchCriteria ['value'] = strtolower ( str_replace ( array (
					'-',
					'_' 
			), '', $consoleProviderName ) );
		...
		if (! $providerMetadata) {
			require_once 'Zend/Tool/Framework/Client/Exception.php';
			throw new Zend_Tool_Framework_Client_Exception ( 'Provider \'' . $consoleProviderFull . '\' is not a valid provider.' );
		}
		...
		if (! $providerSpecialtyMetadata) {
			...
			$providerSpecialtySearchCriteria ['value'] = strtolower ( str_replace ( array (
					'-',
					'_' 
		...
		if (! $providerSpecialtyMetadata) {
			require_once 'Zend/Tool/Framework/Client/Exception.php';
			throw new Zend_Tool_Framework_Client_Exception ( 'Provider \'' . $consoleSpecialtyName . '\' is not a valid specialty.' );
		}
	...
	protected function _parseProviderOptionsPart() {
		...
		$actionableMethodLongParamsMetadata = $this->_manifestRepository->getMetadata ( array_merge ( $searchParams, array (
				'name' => 'actionableMethodLongParams' 
		) ) );
		
		$actionableMethodShortParamsMetadata = $this->_manifestRepository->getMetadata ( array_merge ( $searchParams, array (
		...
		foreach ( $actionableMethodLongParamsMetadata->getValue () as $parameterNameLong => $consoleParameterNameLong ) {
			...
			} elseif (in_array ( $parameterInfo ['type'], array (
					'int',
					'integer',
					'float' 
			) )) {
