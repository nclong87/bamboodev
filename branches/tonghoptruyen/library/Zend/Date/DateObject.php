...
abstract class Zend_Date_DateObject {
	 ...
	 * Table of Monthdays
	 */
	private static $_monthTable = array (
			31,
			28,
			31,
			30,
	...
	protected function mktime($hour, $minute, $second, $month, $day, $year, $gmt = false) {
		...
		if (isset ( self::$_cache )) {
			if (self::$_cacheTags) {
				self::$_cache->save ( serialize ( $date ), $id, array (
						'Zend_Date' 
				) );
			} else {
	...
	protected function date($format, $timestamp = null, $gmt = false) {
		...
		if (empty ( $gmt ) and empty ( $jump )) {
			...
			if (isset ( self::$_cache )) {
				if (self::$_cacheTags) {
					self::$_cache->save ( serialize ( $timestamp ), $idstamp, array (
							'Zend_Date' 
					) );
				} else {
	...
	protected function getDateParts($timestamp = null, $fast = null) {
		...
		if (isset ( self::$_cache )) {
			if (self::$_cacheTags) {
				self::$_cache->save ( serialize ( $array ), $id, array (
						'Zend_Date' 
				) );
			} else {
