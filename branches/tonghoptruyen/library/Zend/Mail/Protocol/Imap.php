...
class Zend_Mail_Protocol_Imap {
	...
	public function connect($host, $port = null, $ssl = false) {
		...
		if (! $this->_socket) {
			...
			throw new Zend_Mail_Protocol_Exception ( 'cannot connect to host; error = ' . $errstr . ' (errno = ' . $errno . ' )' );
		}
	 ...
	 *        	you can filter the response so you get only the
	 *        	given response lines
	 * @param bool $dontParse
	 *        	if true every line is returned unparsed instead of
	 *        	the decoded tokens
	 * @return null bool array success, false if error, null if bad request
	 * @throws Zend_Mail_Protocol_Exception
	 */
	 ...
	 *        	the literal/-s
	 * @return string array literals, literals with newline ar returned
	 ...
	 * @return bool array if error, array with returned information
	 *         otherwise (flags, exists, recent, uidvalidity)
	 * @throws Zend_Mail_Protocol_Exception
	 */
	public function examineOrSelect($command = 'EXAMINE', $box = 'INBOX') {
		$this->sendRequest ( $command, array (
				$this->escapeString ( $box ) 
		), $tag );
	 ...
	 * @return bool array examineOrselect()
	 * @throws Zend_Mail_Protocol_Exception
	 */
	 ...
	 * @return string array only one item of one message is fetched it's returned as string
	 ...
	 * @return bool array flags if $silent is false, else true or false depending on success
	 * @throws Zend_Mail_Protocol_Exception
	 */
	public function store(array $flags, $from, $to = null, $mode = null, $silent = true) {
		...
		$result = $this->requestAndResponse ( 'STORE', array (
				$set,
				$item,
				$flags 
		), $silent );
	...
	public function copy($folder, $from, $to = null) {
		...
		return $this->requestAndResponse ( 'COPY', array (
				$set,
	...
	public function create($folder) {
		return $this->requestAndResponse ( 'CREATE', array (
				$this->escapeString ( $folder ) 
		), true );
	}
	...
	public function delete($folder) {
		return $this->requestAndResponse ( 'DELETE', array (
				$this->escapeString ( $folder ) 
		), true );
	}
