...
class Zend_Form_Element implements Zend_Validate_Interface {
	...
	public function loadDefaultDecorators() {
		...
		if (empty ( $decorators )) {
			$this->addDecorator ( 'ViewHelper' )->addDecorator ( 'Errors' )->addDecorator ( 'Description', array (
					'tag' => 'p',
					'class' => 'description' 
	...
	public function getValue() {
		...
		if ($this->isArray () && is_array ( $valueFiltered )) {
			array_walk_recursive ( $valueFiltered, array (
					$this,
					'_filterValue' 
			) );
		} else {
	 ...
	 *
	 * @param string $name        	
	 * @return Zend_Validate_Interface false if not found, validator otherwise
	 */
