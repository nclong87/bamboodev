...
class Zend_Paginator implements Countable, IteratorAggregate {
	...
	public static function getAdapterLoader() {
		if (self::$_adapterLoader === null) {
			self::$_adapterLoader = new Zend_Loader_PluginLoader ( array (
	...
	public static function getScrollingStyleLoader() {
		if (self::$_scrollingStyleLoader === null) {
			self::$_scrollingStyleLoader = new Zend_Loader_PluginLoader ( array (
	...
	public function clearPageItemCache($pageNumber = null) {
		...
		if (null === $pageNumber) {
			foreach ( self::$_cache->getIdsMatchingTags ( array (
	 ...
	 *
	 * @param integer $relativeItemNumber
	 ...
	 * The current page is used if there's no
	 * page sepcified.
	 *
	...
	public function getItemsByPage($pageNumber) {
		...
		if ($this->_cacheEnabled ()) {
			self::$_cache->save ( $items, $this->_getCacheId ( $pageNumber ), array (
					$this->_getCacheInternalId () 
			) );
		}
	 ...
	 * If none registered, attempts to pull f
	 * rom ViewRenderer.
	 *
