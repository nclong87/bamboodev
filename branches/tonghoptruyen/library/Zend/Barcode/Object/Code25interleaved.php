...
class Zend_Barcode_Object_Code25interleaved extends Zend_Barcode_Object_Code25 {
	...
	protected function _prepareBarcode() {
		...
		for($i = 0; $i < strlen ( $text ); $i += 2) { // Draw 2 chars at a time
			...
			for($ibar = 0; $ibar < 5; $ibar ++) {
				...
				$barWidth = (substr ( $this->_codingMap [$char1], $ibar, 1 )) ? $this->_barThickWidth : $this->_barThinWidth;
				...
				$barWidth = (substr ( $this->_codingMap [$char2], $ibar, 1 )) ? $this->_barThickWidth : $this->_barThinWidth;
				$barcodeTable [] = array (
						0,
						$barWidth,
						0,
	...
	protected function _postDrawBarcode() {
		...
		$point1 = $this->_rotate ( 0, 0 + $this->_barHeight * $this->_factor - 1 );
		$point2 = $this->_rotate ( $this->_calculateWidth () - 1, 0 + $this->_barHeight * $this->_factor - 1 );
		$point3 = $this->_rotate ( $this->_calculateWidth () - 1, 0 + $this->_barHeight * $this->_factor - $width );
		$point4 = $this->_rotate ( 0, 0 + $this->_barHeight * $this->_factor - $width );
		$this->_addPolygon ( array (
				$point1,
				$point2,
				$point3,
