	...
	protected function _drawText() {
		if ($this->_drawText) {
			...
			for($i = 0; $i < $this->_barcodeLength; $i ++) {
				...
				$this->_addText ( $text {$i}, $fontSize * $this->_factor, $this->_rotate ( $leftPosition, ( int ) $this->_withBorder * 2 + $this->_factor * ($this->_barHeight + $fontSize) + 1 ), $this->_font, $this->_foreColor, 'left', - $this->_orientation );
				switch ($i) {
					case 0 :
						$factor = 3;
						break;
