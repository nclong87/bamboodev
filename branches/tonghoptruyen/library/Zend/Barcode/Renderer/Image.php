...
class Zend_Barcode_Renderer_Image extends Zend_Barcode_Renderer_RendererAbstract {
	 ...
	 * @return Zend_Image_Barcode_Abstract @throw Zend_Image_Barcode_Exception
	 */
	 ...
	 *
	 * @param string $value        	
	 * @return Zend_Barcode_RendererAbstract @throw Zend_Barcode_Renderer_Exception
	 */
	...
	protected function _initRenderer() {
		if (! extension_loaded ( 'gd' )) {
			...
			$e = new Zend_Barcode_Exception ( 'Gd extension must be loaded to render barcode as image' );
		...
		if ($this->_resource !== null) {
			...
			$this->_imageBackgroundColor = imagecolorallocate ( $this->_resource, ($backgroundColor & 0xFF0000) >> 16, ($backgroundColor & 0x00FF00) >> 8, $backgroundColor & 0x0000FF );
			$this->_imageForeColor = imagecolorallocate ( $this->_resource, ($foreColor & 0xFF0000) >> 16, ($foreColor & 0x00FF00) >> 8, $foreColor & 0x0000FF );
		} else {
			$width = $barcodeWidth;
	...
	protected function _drawPolygon($points, $color, $filled = true) {
		...
		$allocatedColor = imagecolorallocate ( $this->_resource, ($color & 0xFF0000) >> 16, ($color & 0x00FF00) >> 8, $color & 0x0000FF );
	...
	protected function _drawText($text, $size, $position, $font, $color, $alignment = 'center', $orientation = 0) {
		...
		if (is_numeric ( $font )) {
			...
			imagettftext ( $this->_resource, $size, $orientation, $position [0] - ($width * cos ( pi () * $orientation / 180 )), $position [1] + ($width * sin ( pi () * $orientation / 180 )), $allocatedColor, $font, $text );
		}
	}
}
