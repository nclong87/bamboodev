...
abstract class Zend_View_Abstract implements Zend_View_Interface {
	...
	public function __construct($config = array()) {
		...
		if (array_key_exists ( 'assign', $config ) && is_array ( $config ['assign'] )) {
	 ...
	 * @param string $path        	
	 * @param string $prefix
	 *        	Prefix to use for helper and filter paths
	 * @return Zend_View_Abstract
	 */
	 ...
	 *
	 * @param string $name        	
	 * @return false string if script not found
	 ...
	 *        	Class prefix to use with classes in this
	 ...
	 * @param string $classPrefix
	 *        	The class prefix to apply to all elements in
	 ...
	 *
	 * @param string $name        	
	 * @return string false on failure, path on success
	 */
	...
	public function escape($var) {
		if (in_array ( $this->_escape, array (
				'htmlspecialchars',
				'htmlentities' 
		) )) {
	 ...
	 * @param string $prefix
	 *        	Class prefix to use with path (helpers and filters
	 *        	only)
	 * @return void
	 */
		...
		foreach ( ( array ) $path as $dir ) {
			...
			switch ($type) {
				...
				default :
					...
					array_unshift ( $this->_path [$type], array (
							'prefix' => $prefix,
							'dir' => $dir 
					) );
					break;
