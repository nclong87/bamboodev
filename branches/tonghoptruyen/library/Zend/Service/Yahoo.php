...
class Zend_Service_Yahoo {
	 ...
	 * The most basic query consists simply
	...
	protected function _validateInlinkDataSearch(array $options) {
		...
		if (isset ( $options ['omit_inlinks'] )) {
			$this->_validateInArray ( 'omit_inlinks', $options ['omit_inlinks'], array (
					'none',
					'domain',
					'subdomain' 
			) );
		...
		if (isset ( $options ['coloration'] )) {
			switch ($options ['coloration']) {
				...
				default :
					 ...
					 */
					require_once 'Zend/Service/Exception.php';
					throw new Zend_Service_Exception ( "Invalid value for option 'coloration': " . "{$options['coloration']}" );
			}
		}
	}
	...
	protected function _validateLocalSearch(array $options) {
		...
		if (! in_array ( $options ['sort'], array (
				'relevance',
				'title',
				'distance',
				'rating' 
	...
	protected function _validateNewsSearch(array $options) {
		...
		$this->_validateInArray ( 'sort', $options ['sort'], array (
				'rank',
				'date' 
		) );
		$this->_validateInArray ( 'type', $options ['type'], array (
				'all',
				'any',
				'phrase' 
		) );
	...
	protected function _validateVideoSearch(array $options) {
		...
		if (isset ( $options ['format'] )) {
			$this->_validateInArray ( 'format', $options ['format'], array (
					'any',
					'avi',
					'flash',
					'mpeg',
	...
	protected function _validateWebSearch(array $options) {
		...
		if (isset ( $options ['license'] )) {
			$this->_validateInArray ( 'license', $options ['license'], array (
					'any',
					'cc_any',
					'cc_commercial',
					'cc_modifiable' 
		...
		if (isset ( $options ['region'] )) {
			$this->_validateInArray ( 'region', $options ['region'], array (
					'ar',
					'au',
					'at',
					'br',
