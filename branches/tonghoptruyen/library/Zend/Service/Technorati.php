...
class Zend_Service_Technorati {
	...
	protected function _validateCosmos(array $options) {
		...
		$this->_validateInArrayOption ( 'type', $options, array (
				'link',
				'weblog' 
		) );
	...
	protected function _validateSearch(array $options) {
		...
		$this->_validateInArrayOption ( 'authority', $options, array (
				'n',
				'a1',
				'a4',
				'a7' 
	...
	protected function _validateDailyCounts(array $options) {
		...
		if (isset ( $options ['days'] )) {
			...
			if ($options ['days'] < self::PARAM_DAYS_MIN_VALUE || $options ['days'] > self::PARAM_DAYS_MAX_VALUE) {
				/**
				 *
				 * @see Zend_Service_Technorati_Exception
				 */
				require_once 'Zend/Service/Technorati/Exception.php';
				throw new Zend_Service_Technorati_Exception ( "Invalid value '" . $options ['days'] . "' for 'days' option" );
			}
		}
	}
	...
	protected function _validateInArrayOption($name, $options, array $array) {
		if (isset ( $options [$name] ) && ! in_array ( $options [$name], $array )) {
			 ...
			 */
			require_once 'Zend/Service/Technorati/Exception.php';
			throw new Zend_Service_Technorati_Exception ( "Invalid value '{$options[$name]}' for '$name' option" );
		}
	}
	...
	protected function _validateMandatoryOption($name, $options) {
		if (! isset ( $options [$name] ) || ! trim ( $options [$name] )) {
			 ...
			 */
			require_once 'Zend/Service/Technorati/Exception.php';
			throw new Zend_Service_Technorati_Exception ( "Empty value for '$name' option" );
		}
	}
	...
	protected function _validateOptionFormat(array $options) {
		if (isset ( $options ['format'] ) && $options ['format'] != 'xml') {
			 ...
			 */
			require_once 'Zend/Service/Technorati/Exception.php';
			throw new Zend_Service_Technorati_Exception ( "Invalid value '" . $options ['format'] . "' for 'format' option. " . "Zend_Service_Technorati supports only 'xml'" );
		}
	}
	...
	protected function _validateOptionLimit(array $options) {
		...
		if ($options ['limit'] < self::PARAM_LIMIT_MIN_VALUE || $options ['limit'] > self::PARAM_LIMIT_MAX_VALUE) {
			/**
			 *
			 * @see Zend_Service_Technorati_Exception
			 */
			require_once 'Zend/Service/Technorati/Exception.php';
			throw new Zend_Service_Technorati_Exception ( "Invalid value '" . $options ['limit'] . "' for 'limit' option" );
		}
	}
	...
	protected function _validateOptionStart(array $options) {
		...
		if ($options ['start'] < self::PARAM_START_MIN_VALUE) {
			 ...
			 */
			require_once 'Zend/Service/Technorati/Exception.php';
			throw new Zend_Service_Technorati_Exception ( "Invalid value '" . $options ['start'] . "' for 'start' option" );
		}
	}
	...
	protected function _compareOptions(array $options, array $validOptions) {
		...
		if ($difference) {
			 ...
			 */
			require_once 'Zend/Service/Technorati/Exception.php';
			throw new Zend_Service_Technorati_Exception ( "The following parameters are invalid: '" . implode ( "', '", $difference ) . "'" );
		}
	}
