...
class Zend_Service_WindowsAzure_Storage_Queue extends Zend_Service_WindowsAzure_Storage {
	...
	public function createQueue($queueName = '', $metadata = array()) {
		...
		if ($response->isSuccessful ()) {
			return new Zend_Service_WindowsAzure_Storage_QueueInstance ( $queueName, $metadata );
		} else {
	...
	public function getQueue($queueName = '') {
		...
		if ($response->isSuccessful ()) {
			...
			// Return queue
			$queue = new Zend_Service_WindowsAzure_Storage_QueueInstance ( $queueName, $metadata );
		...
		if ($response->isSuccessful ()) {
			...
			if ($xmlQueues !== null) {
				for($i = 0; $i < count ( $xmlQueues ); $i ++) {
					$queues [] = new Zend_Service_WindowsAzure_Storage_QueueInstance ( ( string ) $xmlQueues [$i]->Name, $this->_parseMetadataElement ( $xmlQueues [$i] ) );
				}
			}
