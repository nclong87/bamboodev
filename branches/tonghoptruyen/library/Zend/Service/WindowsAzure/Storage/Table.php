...
class Zend_Service_WindowsAzure_Storage_Table extends Zend_Service_WindowsAzure_Storage_BatchStorageAbstract {
	...
	public function listTables($nextTableName = '') {
		...
		if ($response->isSuccessful ()) {
			...
			foreach ( $entries as $entry ) {
				...
				$returnValue [] = new Zend_Service_WindowsAzure_Storage_TableInstance ( ( string ) $entry->id, $tableName, ( string ) $entry->link ['href'], ( string ) $entry->updated );
			}
	...
	public function createTable($tableName = '') {
		...
		if ($response->isSuccessful ()) {
			...
			return new Zend_Service_WindowsAzure_Storage_TableInstance ( ( string ) $entry->id, $tableName, ( string ) $entry->link ['href'], ( string ) $entry->updated );
		} else {
			throw new Zend_Service_WindowsAzure_Exception ( $this->_getErrorMessage ( $response, 'Resource could not be accessed.' ) );
		}
	}
	...
	public function retrieveEntityById($tableName = '', $partitionKey = '', $rowKey = '', $entityClass = 'Zend_Service_WindowsAzure_Storage_DynamicTableEntity') {
		...
		$result = $this->retrieveEntities ( $this->select ()->from ( $tableName )->wherePartitionKey ( $partitionKey )->whereRowKey ( $rowKey ), '', $entityClass );
	 ...
	 *
	 * @param string $tableName|Zend_Service_WindowsAzure_Storage_TableEntityQuery
	 *        	Table name -or- Zend_Service_WindowsAzure_Storage_TableEntityQuery instance
	 * @param string $filter
