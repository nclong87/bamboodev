...
class Zend_Service_Amazon_S3 extends Zend_Service_Amazon_Abstract {
	...
	public function isBucketAvailable($bucket) {
		$response = $this->_makeRequest ( 'HEAD', $bucket, array (
				'max-keys' => 0 
		) );
	 ...
	 * All objects in the bucket must be removed prior
	...
	public function getObject($object, $paidobject = false) {
		$object = $this->_fixupObjectName ( $object );
		if ($paidobject) {
			$response = $this->_makeRequest ( 'GET', $object, null, array (
					self::S3_REQUESTPAY_HEADER => 'requester' 
			) );
		} else {
	...
	public function _makeRequest($method, $path = '', $params = null, $headers = array(), $data = null) {
		...
		/*
		 * $client->setHeaders(array('Content-MD5' => null, 'Content-Encoding' => null, 'Expect' => null, 'Range' => null, 'x-amz-acl' => null, 'x-amz-copy-source' => null, 'x-amz-metadata-directive' => null));
		 */
