...
class Zend_Pdf {
	 ...
	 * @var array
	 */
	protected static $_inheritableAttributes = array (
			'Resources',
			'MediaBox',
			'CropBox',
			'Rotate' 
	...
	public function setNamedDestination($name, $destination = null) {
		if ($destination !== null && ! $destination instanceof Zend_Pdf_Action_GoTo && ! $destination instanceof Zend_Pdf_Destination_Explicit) {
			require_once 'Zend/Pdf/Exception.php';
	 ...
	 * @param boolean $refreshPagesHash
	...
	public function render($newSegmentOnly = false, $outputStream = null) {
		...
		if ($outputStream !== null) {
			...
			$pdfSegmentBlocks = ($newSegmentOnly) ? array () : array (
					$this->_trailer->getPDFString () 
			);
		}
