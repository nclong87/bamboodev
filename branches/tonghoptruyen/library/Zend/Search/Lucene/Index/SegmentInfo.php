...
class Zend_Search_Lucene_Index_SegmentInfo implements Zend_Search_Lucene_Index_TermsStream_Interface {
	...
	public function __construct(Zend_Search_Lucene_Storage_Directory $directory, $name, $docCount, $delGen = 0, $docStoreOptions = null, $hasSingleNormFile = false, $isCompound = null) {
		...
		for($count = 0; $count < $fieldsCount; $count ++) {
			...
			$this->_fields [$count] = new Zend_Search_Lucene_Index_FieldInfo ( $fieldName, $fieldBits & 0x01 /* field is indexed */,
                                                                            $count, $fieldBits & 0x02 /* termvectors are stored */,
	...
	public function openCompoundFile($extension, $shareHandler = true) {
		if (($extension == '.fdx' || $extension == '.fdt') && $this->_usesSharedDocStore) {
			...
			if (! isset ( $this->_sharedDocStoreOptions ['files'] [$fdxFName] )) {
				require_once 'Zend/Search/Lucene/Exception.php';
				throw new Zend_Search_Lucene_Exception ( 'Shared doc storage segment compound file doesn\'t contain ' . $fdxFName . ' file.' );
			}
			if (! isset ( $this->_sharedDocStoreOptions ['files'] [$fdtFName] )) {
				require_once 'Zend/Search/Lucene/Exception.php';
				throw new Zend_Search_Lucene_Exception ( 'Shared doc storage segment compound file doesn\'t contain ' . $fdtFName . ' file.' );
			}
		...
		if (! isset ( $this->_segFiles [$filename] )) {
			require_once 'Zend/Search/Lucene/Exception.php';
			throw new Zend_Search_Lucene_Exception ( 'Segment compound file doesn\'t contain ' . $filename . ' file.' );
		}
	...
	public function compoundFileLength($extension) {
		if (($extension == '.fdx' || $extension == '.fdt') && $this->_usesSharedDocStore) {
			...
			if (! isset ( $this->_sharedDocStoreOptions ['fileSizes'] [$filename] )) {
				require_once 'Zend/Search/Lucene/Exception.php';
				throw new Zend_Search_Lucene_Exception ( 'Shared doc store compound file doesn\'t contain ' . $filename . ' file.' );
			}
	...
	private function _getFieldPosition($fieldNum) {
		...
		return isset ( $this->_fieldsDicPositions [$fieldNum] ) ? $this->_fieldsDicPositions [$fieldNum] : $fieldNum;
	}
	...
	public function normVector($fieldName) {
		...
		if ($fieldNum == - 1 || ! ($this->_fields [$fieldNum]->isIndexed)) {
			...
			return str_repeat ( chr ( $similarity->encodeNorm ( $similarity->lengthNorm ( $fieldName, 0 ) ) ), $this->_docCount );
		}
	 ...
	 */
	{
		...
		$this->_termCount = $this->_termNum = $this->_tisFile->readLong (); // Read terms count
	...
	public function skipTo(Zend_Search_Lucene_Index_Term $prefix) {
		...
		while ( $this->_lastTerm !== null ) {
			if (strcmp ( $this->_lastTerm->field, $prefix->field ) > 0 || ($prefix->field == $this->_lastTerm->field && strcmp ( $this->_lastTerm->text, $prefix->text ) >= 0)) {
