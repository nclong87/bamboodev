...
class Zend_Search_Lucene_Storage_File_Memory extends Zend_Search_Lucene_Storage_File {
	...
	public function writeInt($value) {
		...
		$this->_data .= chr ( $value >> 24 & 0xFF ) . chr ( $value >> 16 & 0xFF ) . chr ( $value >> 8 & 0xFF ) . chr ( $value & 0xFF );
	...
	public function writeLong($value) {
		...
		if (PHP_INT_SIZE > 4) {
			settype ( $value, 'integer' );
			$this->_data .= chr ( $value >> 56 & 0xFF ) . chr ( $value >> 48 & 0xFF ) . chr ( $value >> 40 & 0xFF ) . chr ( $value >> 32 & 0xFF ) . chr ( $value >> 24 & 0xFF ) . chr ( $value >> 16 & 0xFF ) . chr ( $value >> 8 & 0xFF ) . chr ( $value & 0xFF );
		} else {
