...
class Zend_Validate_NotEmpty extends Zend_Validate_Abstract {
	...
	public function isValid($value) {
		if ($value !== null && ! is_string ( $value ) && ! is_int ( $value ) && ! is_float ( $value ) && ! is_bool ( $value ) && ! is_array ( $value ) && ! is_object ( $value )) {
		...
		if ($type >= self::OBJECT_STRING) {
			...
			if ((is_object ( $value ) && (! method_exists ( $value, '__toString' ))) || (is_object ( $value ) && (method_exists ( $value, '__toString' )) && ((( string ) $value) == ""))) {
				$this->_error ( self::IS_EMPTY );
				return false;
			}
		}
