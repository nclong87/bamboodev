...
class Zend_Queue_Adapter_Db extends Zend_Queue_Adapter_AdapterAbstract {
	...
	/*
	 * Get an array of all available queues Not all adapters support getQueues(), use isSupported('getQueues') to determine if the adapter supports this feature. @return array @throws Zend_Queue_Exception - database error
	 */
	public function getQueues() {
		$query = $this->_queueTable->select ();
		$query->from ( $this->_queueTable, array (
				'queue_id',
				'queue_name' 
		) );
	...
	public function receive($maxMessages = null, $timeout = null, Zend_Queue $queue = null) {
		...
		try {
			if ($maxMessages > 0) { // ZF-7666 LIMIT 0 clause not included.
				...
				$query->from ( $info ['name'], array (
						'*' 
				) )->where ( 'queue_id=?', $this->getQueueId ( $queue->getName () ) )->where ( 'handle IS NULL OR timeout+' . ( int ) $timeout . ' < ' . ( int ) $microtime )->limit ( $maxMessages );
