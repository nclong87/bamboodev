...
abstract class Zend_Pdf_Font {
	...
	/**
	 * Full font name.
	 * Usually a combination of the {@link NAME_FAMILY} and
	 ...
	 * This is the name used to identify fonts
	 ...
	 * This is distinct from the {@link NAME_COPYRIGHT}.
	 */
	 ...
	 * May contain revision information, usage
	 ...
	 * Some fonts may contain a unique serial number
	 ...
	 * Used by some fonts to work around a Microsoft
	 ...
	 * A more descriptive string than {@link NAME_STYLE}.
	 */
	...
	public static function fontWithName($name, $embeddingOptions = 0) {
		...
		/*
		 * Not an existing font and no mapping in the config file. Check to see if this is one of the standard 14 PDF fonts.
		 */
		switch ($name) {
	...
	public static function fontWithPath($filePath, $embeddingOptions = 0) {
		...
		/*
		 * Create a file parser data source object for this file. File path and access permission checks are handled here.
		 */
		require_once 'Zend/Pdf/FileParserDataSource/File.php';
		$dataSource = new Zend_Pdf_FileParserDataSource_File ( $filePath );
		
		/*
		 * Attempt to determine the type of font. We can't always trust file extensions, but try that first since it's fastest.
		 */
		$fileExtension = strtolower ( pathinfo ( $filePath, PATHINFO_EXTENSION ) );
		
		/*
		 * If it turns out that the file is named improperly and we guess the wrong type, we'll get null instead of a font object.
		 */
		...
		if ($font === null) {
			/*
			 * There was no match for the file extension or the extension was wrong. Attempt to detect the type of font by actually parsing it. We'll do the checks in order of most likely format to try to reduce the detection time.
			 */
		...
		if ($font !== null) {
			/*
			 * Parsing was successful. Add this font instance to the cache arrays and return it for use.
			 */
	...
	protected static function _extractTrueTypeFont($dataSource, $embeddingOptions) {
		try {
			...
			/*
			 * The following exception codes suggest that this isn't really a TrueType font. If we caught such an exception, simply return null. For all other cases, it probably is a TrueType font but has a problem; throw the exception again.
			 */
			$fontParser = null;
			require_once 'Zend/Pdf/Exception.php';
			switch ($e->getCode ()) {
