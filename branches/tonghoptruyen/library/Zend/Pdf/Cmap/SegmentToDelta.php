...
class Zend_Pdf_Cmap_SegmentToDelta extends Zend_Pdf_Cmap {
	...
	public function glyphNumbersForCharacters($characterCodes) {
		...
		foreach ( $characterCodes as $key => $characterCode ) {
			...
			/*
			 * Now do a binary search to find the first segment whose end code is greater or equal to our character code. No matter the number of segments (there may be hundreds in a large font), we will only need to perform $this->_searchIterations.
			 */
			...
			/*
			 * If the segment's start code is greater than our character code, that character is not represented in this font. Move on.
			 */
			...
			if ($this->_segmentTableIdRangeOffsets [$subtableIndex] == 0) {
				/*
				 * This segment uses a simple mapping from character code to glyph number.
				 */
				$glyphNumbers [$key] = ($characterCode + $this->_segmentTableIdDeltas [$subtableIndex]) % 65536;
			} else {
				/*
				 * This segment relies on the glyph index array to determine the glyph number. The calculation below determines the correct index into that array. It's a little odd because the range offset in the font file is designed to quickly provide an address of the index in the raw binary data instead of the index itself. Since we've parsed the data into arrays, we must process it a bit differently.
				 */
				$glyphIndex = ($characterCode - $this->_segmentTableStartCodes [$subtableIndex] + $this->_segmentTableIdRangeOffsets [$subtableIndex] - $this->_segmentCount + $subtableIndex - 1);
	...
	public function glyphNumberForCharacter($characterCode) {
		/*
		 * This code is pretty much a copy of glyphNumbersForCharacters(). See that method for inline documentation.
		 */
		if ($characterCode > 0xffff) {
	...
	public function getCoveredCharactersGlyphs() {
		...
		for($segmentNum = 1; $segmentNum <= $this->_segmentCount; $segmentNum ++) {
			if ($this->_segmentTableIdRangeOffsets [$segmentNum] == 0) {
				...
				for($code = $this->_segmentTableStartCodes [$segmentNum]; $code <= $this->_segmentTableEndCodes [$segmentNum]; $code ++) {
	...
	public function __construct($cmapData) {
		...
		/*
		 * Mapping tables should be language-independent. The font may not work as expected if they are not. Unfortunately, many font files in the wild incorrectly record a language ID in this field, so we can't call this a failure.
		 */
		...
		/*
		 * These two values are stored premultiplied by two which is convienent when using the binary data directly, but we're parsing it out to native PHP data types, so divide by two.
		 */
		...
		/*
		 * The range offset helps determine the index into the glyph index array. Like the segment count and search range above, it's stored as a byte multiple in the font, so divide by two as we extract the values.
		 */
		for($i = 1; $i <= $this->_segmentCount; $i ++, $offset += 2) {
		...
		/*
		 * The size of the glyph index array varies by font and depends on the extent of the usage of range offsets versus deltas. Some fonts may not have any entries in this array.
		 */
		...
		/*
		 * Sanity check: After reading all of the data, we should be at the end of the table.
		 */
		if ($offset != $length) {
			require_once 'Zend/Pdf/Exception.php';
