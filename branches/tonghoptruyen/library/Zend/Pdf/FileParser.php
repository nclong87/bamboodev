 ...
 * @version    $Id: FileParser.php 23775 2011-03-01 17:25:24Z ralph $
 */
...
abstract class Zend_Pdf_FileParser {
	 ...
	 *        	(optional) Big- or little-endian byte order.
	...
	public function readInt($size, $byteOrder = Zend_Pdf_FileParser::BYTE_ORDER_BIG_ENDIAN) {
		...
		/*
		 * unpack() will not work for this method because it always works in the host byte order for signed integers. It also does not allow for variable integer sizes.
		 */
		if ($byteOrder == Zend_Pdf_FileParser::BYTE_ORDER_BIG_ENDIAN) {
			...
			if (($number & 0x80) == 0x80) {
				...
				/*
				 * Now turn this back into a negative number by taking the two's complement (we didn't add one above so won't subtract it below). This works reliably on both 32- and 64-bit systems.
				 */
				$number = ~ $number;
			} else {
				for($i = 1; $i < $size; $i ++) {
			...
			require_once 'Zend/Pdf/Exception.php';
			throw new Zend_Pdf_Exception ( "Invalid byte order: $byteOrder", Zend_Pdf_Exception::INVALID_BYTE_ORDER );
		}
		return $number;
	}
	...
	public function readUInt($size, $byteOrder = Zend_Pdf_FileParser::BYTE_ORDER_BIG_ENDIAN) {
		...
		$bytes = $this->_dataSource->readBytes ( $size );
		/*
		 * unpack() is a bit heavyweight for this simple conversion. Just work the bytes directly.
		 */
		if ($byteOrder == Zend_Pdf_FileParser::BYTE_ORDER_BIG_ENDIAN) {
			$number = ord ( $bytes [0] );
			for($i = 1; $i < $size; $i ++) {
	 ...
	 * @throws Zend_Pdf_Exception
	 */
	public function readFixed($mantissaBits, $fractionBits, $byteOrder = Zend_Pdf_FileParser::BYTE_ORDER_BIG_ENDIAN) {
	 ...
	 *        	(optional) Desired resulting character set.
	 *        	You may use any character set supported by {@link iconv()}. If omitted,
	 *        	uses 'current locale'.
	 * @return string
	 * @throws Zend_Pdf_Exception
	 */
	public function readStringUTF16($byteCount, $byteOrder = Zend_Pdf_FileParser::BYTE_ORDER_BIG_ENDIAN, $characterSet = '') {
		if ($byteCount == 0) {
			return '';
		}
	 ...
	 *        	You may use any character set supported by {@link iconv()}. If omitted,
	 *        	uses 'current locale'.
	 * @param integer $lengthBytes
	 *        	(optional) Number of bytes that make up the
	 *        	length. Default is 1.
	 * @return string
	 * @throws Zend_Pdf_Exception
	 */
