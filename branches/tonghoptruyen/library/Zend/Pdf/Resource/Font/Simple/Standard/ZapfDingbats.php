...
class Zend_Pdf_Resource_Font_Simple_Standard_ZapfDingbats extends Zend_Pdf_Resource_Font_Simple_Standard {
	...
	public function __construct() {
		...
		$this->_cmap = Zend_Pdf_Cmap::cmapWithTypeData ( Zend_Pdf_Cmap::TYPE_BYTE_ENCODING_STATIC, $cmapData );
		...
		/*
		 * This font has a built-in custom character encoding method. Don't override with WinAnsi like the other built-in fonts or else it will not work as expected.
		 */
	...
	public function encodeString($string, $charEncoding) {
		/*
		 * This isn't the optimal time to perform this conversion, but it must live here until the remainder of the layout code is completed. This, and the $charEncoding parameter, will go away soon...
		 */
		...
		for($i = 0; $i < strlen ( $string ); $i ++) {
			...
			if (isset ( $this->_toFontEncoding [$characterCode] )) {
				...
				/*
				 * For now, mimic the behavior in Zend_Pdf_Font::encodeString() where unknown characters are removed completely. This is not perfect, but we should be consistent. In a future revision, we will use the well-known substitution character 0x1a (Control-Z).
				 */
			}
		}
	...
	public function decodeString($string, $charEncoding) {
		...
		for($i = 0; $i < strlen ( $string ); $i ++) {
			...
			if (isset ( $this->_fromFontEncoding [$characterCode] )) {
				...
				/*
				 * For now, mimic the behavior in Zend_Pdf_Font::encodeString() where unknown characters are removed completely. This is not perfect, but we should be consistent. In a future revision, we will use the Unicode substitution character (U+FFFD).
				 */
			}
		}
		if ($charEncoding != 'UTF-16BE') {
	 ...
	 * @param string $string        	
	 * @param string $charEncoding
	 *        	(optional) Character encoding of source
	 *        	string. Defaults to current locale.
	 * @return string
	 */
	public function toUnicode($string, $charEncoding = '') {
		/*
		 * When using these faked strings, the closest match to the font's internal encoding is ISO-8859-1.
		 */
