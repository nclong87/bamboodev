...
class Zend_Currency {
	 ...
	 *        	OPTIONAL Options array or currency short name
	...
	private function _checkParams($currency = null, $locale = null) {
		// Manage the params
		if ((empty ( $locale )) and (! empty ( $currency )) and (Zend_Locale::isLocale ( $currency, true, false ))) {
			$locale = $currency;
			$currency = null;
		}
	 ...
	 * when no symbol is available it returns the currency shortname (f.e.
	 ...
	 * a region name should be 2 charachters only (f.e.
	 * EG, DE, US)
	 ...
	 * Adds a currency
	 *
	 * @param float|integer|Zend_Currency $value
	 *        	Add this value to currency
	 * @param string|Zend_Currency $currency
	 *        	The currency to add
	 * @return Zend_Currency
	 */
	 ...
	 *
	 * @param string|Zend_Currency_CurrencyInterface $service
	 *        	Service class
	 * @return Zend_Currency
	 */
