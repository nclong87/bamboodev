...
class Zend_Wildfire_Protocol_JsonStream {
	...
	protected function _encode($value) {
		return Zend_Json::encode ( $value, true, array (
	...
	public function getPayload(Zend_Wildfire_Channel_Interface $channel) {
		...
		foreach ( $this->_messages as $structure_uri => $plugin_messages ) {
			...
			$payload [] = array (
					$protocol_index . '-Structure-' . $structure_index,
					$structure_uri 
			);
			...
			foreach ( $plugin_messages as $plugin_uri => $messages ) {
				...
				$payload [] = array (
						$protocol_index . '-Plugin-' . $plugin_index,
						$plugin_uri 
				);
				...
				foreach ( $messages as $message ) {
					...
					for($i = 0; $i < count ( $parts ); $i ++) {
						...
						if ($part) {
							...
							if (count ( $parts ) > 2) {
								$msg = (($i == 0) ? strlen ( $message ) : '') . '|' . $part . '|' . (($i < count ( $parts ) - 2) ? '\\' : '');
							} else {
								$msg = strlen ( $part ) . '|' . $part . '|';
							}
							
							$payload [] = array (
									$protocol_index . '-' . $structure_index . '-' . $plugin_index . '-' . $message_index,
									$msg 
							);
