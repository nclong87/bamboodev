...
class Zend_Amf_Server implements Zend_Server_Interface {
	...
	public function __construct() {
		Zend_Amf_Parse_TypeLoader::setResourceLoader ( new Zend_Loader_PluginLoader ( array (
				"Zend_Amf_Parse_Resource" => "Zend/Amf/Parse/Resource" 
		) ) );
	}
	...
	protected function _handleAuth($userid, $password) {
		...
		if ($result->isValid ()) {
			...
			require_once 'Zend/Amf/Server/Exception.php';
			throw new Zend_Amf_Server_Exception ( "Authentication failed: " . join ( "\n", $result->getMessages () ), $result->getCode () );
		}
	}
	...
	protected function _handle(Zend_Amf_Request $request) {
		...
		if (isset ( $headers [Zend_Amf_Constants::CREDENTIALS_HEADER] ) && isset ( $headers [Zend_Amf_Constants::CREDENTIALS_HEADER]->userid ) && isset ( $headers [Zend_Amf_Constants::CREDENTIALS_HEADER]->password )) {
			try {
	 ...
	 *
	 * @param null|Zend_Amf_Request $request
	 *        	Optional
	 * @return Zend_Amf_Response
	 */
	 ...
	 *        	Optional
	 * @param mixed $arg
	 *        	Optional arguments to pass to a method
	 * @return Zend_Amf_Server
