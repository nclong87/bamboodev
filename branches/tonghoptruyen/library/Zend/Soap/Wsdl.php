...
class Zend_Soap_Wsdl {
	...
	public function setComplexTypeStrategy($strategy) {
		if ($strategy === true) {
			...
			if (class_exists ( $strategy )) {
				...
				throw new Zend_Soap_Wsdl_Exception ( sprintf ( "Strategy with name '%s does not exist.", $strategy ) );
			}
		}
	...
	public function addBindingOperation($binding, $name, $input = false, $output = false, $fault = false) {
		...
		if (is_array ( $fault )) {
			...
			/**
			 * Note.
			 * Do we really need name attribute to be also set at wsdl:fault node???
