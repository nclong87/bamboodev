...
class Zend_Db_Adapter_Sqlsrv extends Zend_Db_Adapter_Abstract {
	...
	public function isConnected() {
		return (is_resource ( $this->_connection ) && (get_resource_type ( $this->_connection ) == 'SQL Server Connection'));
	}
	...
	public function insert($table, array $bind) {
		...
		// build the statement
		$sql = "INSERT INTO " . $this->quoteIdentifier ( $table, true ) . ' (' . implode ( ', ', $cols ) . ') ' . 'VALUES (' . implode ( ', ', $vals ) . ')' . ' ' . $this->_lastInsertSQL;
	...
	public function describeTable($tableName, $schemaName = null) {
		...
		$sql = "exec sp_pkeys @table_owner = " . $tableOwner . ", @table_name = " . $this->quoteIdentifier ( $tableName, true );
		$stmt = $this->query ( $sql );
