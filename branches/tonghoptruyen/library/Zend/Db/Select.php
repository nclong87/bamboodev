...
class Zend_Db_Select {
	 ...
	 *
	 * @return string null object as a SELECT string. (or null if a string cannot be produced.)
	 */
		...
		foreach ( $this->_parts [self::COLUMNS] as $columnEntry ) {
			...
			if ($column instanceof Zend_Db_Expr) {
				...
				if (empty ( $correlationName )) {
					...
					$columns [] = $this->_adapter->quoteColumnAs ( array (
							$correlationName,
							$column 
					), $alias, true );
				}
	...
	protected function _renderFrom($sql) {
		/*
		 * If no table specified, use RDBMS-dependent solution for table-less query. e.g. DUAL in Oracle.
		 */
		if (empty ( $this->_parts [self::FROM] )) {
