...
class Zend_Translate_Adapter_Xliff extends Zend_Translate_Adapter {
	 ...
	 *
	 * @param string $locale
	 *        	Locale/Language to add data for, identical with locale identifier,
	 *        	see Zend_Locale for more information
	 * @param string $filename
	 *        	XLIFF file to add, full path must be given for access
	 * @param array $option
	...
	private function _endElement($file, $name) {
		if (($this->_stag === true) and ($name !== 'source')) {
			...
			switch (strtolower ( $name )) {
				...
				case 'source' :
					if ($this->_useId) {
						if (! empty ( $this->_scontent ) && ! empty ( $this->_langId ) && ! isset ( $this->_data [$this->_source] [$this->_langId] )) {
							$this->_data [$this->_source] [$this->_langId] = $this->_scontent;
						}
					} else {
						if (! empty ( $this->_scontent ) && ! isset ( $this->_data [$this->_source] [$this->_scontent] )) {
				...
				case 'target' :
					if ($this->_useId) {
						if (! empty ( $this->_tcontent ) && ! empty ( $this->_langId ) && ! isset ( $this->_data [$this->_target] [$this->_langId] )) {
							$this->_data [$this->_target] [$this->_langId] = $this->_tcontent;
						}
					} else {
						if (! empty ( $this->_tcontent ) && ! empty ( $this->_scontent ) && ! isset ( $this->_data [$this->_target] [$this->_scontent] )) {
