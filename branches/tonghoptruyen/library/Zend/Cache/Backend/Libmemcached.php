...
class Zend_Cache_Backend_Libmemcached extends Zend_Cache_Backend implements Zend_Cache_Backend_ExtendedInterface {
	 ...
	 * 
	 * @see http://php.net/manual/memcached.constants.php - The option name can be the name of the constant without the prefix 'OPT_'
	 ...
	 * @param string $id
	 *        	Cache id
	 * @return int false cache is not available) or "last modified" timestamp (int) of the available cache record
	 */
	...
	public function save($data, $id, $tags = array(), $specificLifetime = false) {
		...
		$result = @$this->_memcache->set ( $id, array (
				$data,
				time (),
				$lifetime 
		), $lifetime );
	...
	public function touch($id, $extraLifetime) {
		...
		if (isset ( $tmp [0], $tmp [1], $tmp [2] )) {
			...
			if (! ($result = $this->_memcache->replace ( $id, array (
					$data,
					time (),
					$newLifetime 
			), $newLifetime ))) {
