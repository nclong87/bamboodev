	...
	public static function location($self, $url = null) {
		...
		if ($xhr->getLastResponse ()->isSuccessful ()) {
			...
			if (isset ( $self->document->WebBrowserCallback ))
				phpQuery::callbackRun ( $self->document->WebBrowserCallback, array (
						$return 
				) );
		}
		return $return;
...
class phpQueryPlugin_WebBrowser {
	...
	public static function browserGet($url, $callback, $param1 = null, $param2 = null, $param3 = null) {
		...
		if ($xhr->getLastResponse ()->isSuccessful ()) {
			phpQuery::callbackRun ( $callback, array (
					self::browserReceive ( $xhr )->WebBrowser () 
			), $paramStructure );
			// phpQuery::callbackRun($callback, array(
	...
	public static function browser($ajaxSettings, $callback, $param1 = null, $param2 = null, $param3 = null) {
		...
		$xhr = phpQuery::ajax ( self::ajaxSettingsPrepare ( $ajaxSettings ) );
		$paramStructure = null;
		if (func_num_args () > 2) {
			$paramStructure = func_get_args ();
			$paramStructure = array_slice ( $paramStructure, 2 );
	...
	public static function hadleClick($e, $callback = null) {
		...
		if ($node->is ( 'a[href]' )) {
			...
			$node->parents ( 'form' )->trigger ( 'submit', array (
					$e 
			) );
	}
	/**
	...
	public static function handleSubmit($e, $callback = null) {
		...
		if (! $node->is ( 'form' ) || ! $node->is ( '[action]' ))
			...
			// TODO document.location
		$xhr = isset ( $node->document->xhr ) ? $node->document->xhr : null;
		$submit = pq ( $e->relatedTarget )->is ( ':submit' ) ? $e->relatedTarget
				// will this work ?
//			: $node->find(':submit:first')->get(0);
			: $node : 
->find ( '*:submit:first' )->get ( 0 );
		$data = array ();
		foreach ( $node->serializeArray ( $submit ) as $r )
			...
			$data [$r ['name']] = $r ['value'];
		$options = array (
				'type' => $node->attr ( 'method' ) ? $node->attr ( 'method' ) : 'GET',
