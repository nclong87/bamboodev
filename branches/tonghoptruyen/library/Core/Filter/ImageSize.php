 ...
 */
class zendcms_Filter_ImageSize implements Zend_Filter_Interface {
	/**
	...
	/**
	 * Output filetype.
	 * This is one of the following: gif, jpeg, png, auto.
	...
	public function getThumbnailPath($filename) {
		return $this->getThumbnailDirectory () . DIRECTORY_SEPARATOR . $this->getThumbnailBasename ( $filename );
	}
	...
	public function getThumbnailBasename($filename) {
		...
		/*
		 * return sprintf('%s-%sx%s%s', $basename, $this->getWidth(), $this->getHeight(), $ext );
		 */
