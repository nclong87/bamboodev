...
class Zend_Text_Figlet {
	 ...
	 *
	 * @param boolean $handleParagraphs
	 ...
	 * 0 stands for left aligned, 1 for centered and 2
	 * for right aligned.
	 *
	 * @param integer $justification
	 ...
	 *        	Output with which should be used for word
	 ...
	 * current smushmode.
	 * Returns smushed character or false if no smushing can
	 * be done.
	 *
	...
	protected function _loadFont($fontFile) {
		...
		if (substr ( $fontFile, - 3 ) === '.gz') {
			if (! function_exists ( 'gzcompress' )) {
				require_once 'Zend/Text/Figlet/Exception.php';
				throw new Zend_Text_Figlet_Exception ( 'GZIP library is required for ' . 'gzip compressed font files' );
			}
		...
		while ( ! feof ( $fp ) ) {
			...
			} else if (substr ( $uniCode, 0, 1 ) === '0' and $uniCode !== '0' or substr ( $uniCode, 0, 2 ) === '-0') {
	...
	protected function _uniOrd($c) {
		...
		if ($h <= 0x7F) {
			...
			$ord = (($h & 0x0F) << 18 | (ord ( $c [1] ) & 0x3F) << 12 | (ord ( $c [2] ) & 0x3F) << 6 | (ord ( $c [3] ) & 0x3F));
		} else {
			$ord = 0;
		}
