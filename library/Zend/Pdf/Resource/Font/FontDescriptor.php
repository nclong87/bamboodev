 ...
 * @version    $Id: FontDescriptor.php 23775 2011-03-01 17:25:24Z ralph $
 */
...
class Zend_Pdf_Resource_Font_FontDescriptor {
	...
	static public function factory(Zend_Pdf_Resource_Font $font, Zend_Pdf_FileParser_Font_OpenType $fontParser, $embeddingOptions) {
		/*
		 * The font descriptor object contains the rest of the font metrics and the information about the embedded font program (if applicible).
		 */
		...
		/*
		 * The font flags value is a bitfield that describes the stylistic attributes of the font. We will set as many of the bits as can be determined from the font parser.
		 */
		$flags = 0;
		...
		$fontBBox = array (
				new Zend_Pdf_Element_Numeric ( $font->toEmSpace ( $fontParser->xMin ) ),
		 ...
		 * file.
		 * For now, record zero which is interpreted as 'unknown'.
		 * 
		...
		if (! ($embeddingOptions & Zend_Pdf_Font::EMBED_DONT_EMBED)) {
			...
			if (! $fontParser->isEmbeddable) {
				...
				if (! ($embeddingOptions & Zend_Pdf_Font::EMBED_DONT_COMPRESS)) {
					/*
					 * Compress the font file using Flate. This generally cuts file sizes by about half!
					 */
