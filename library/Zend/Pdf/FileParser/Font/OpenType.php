...
abstract class Zend_Pdf_FileParser_Font_OpenType extends Zend_Pdf_FileParser_Font {
	...
	protected function _parseTableDirectory() {
		...
		/*
		 * The directory contains four values: the name of the table, checksum, offset to the table from the beginning of the font, and actual data length of the table.
		 */
		for($tableIndex = 0; $tableIndex < $tableCount; $tableIndex ++) {
			$tableName = $this->readBytes ( 4 );
			
			/*
			 * We ignore the checksum here for two reasons: First, the PDF viewer will do this later anyway; Second, calculating the checksum would require unsigned integers, which PHP does not currently provide. We may revisit this in the future.
			 */
			$this->skipBytes ( 4 );
	...
	protected function _parseHeadTable() {
		...
		if ($magicNumber != 0x5f0f3cf5) {
			require_once 'Zend/Pdf/Exception.php';
			throw new Zend_Pdf_Exception ( 'Wrong magic number. Expected: 0x5f0f3cf5; actual: ' . sprintf ( '%x', $magicNumber ), Zend_Pdf_Exception::BAD_MAGIC_NUMBER );
		}
		
		/*
		 * Most of the flags we ignore, but there are a few values that are useful for our layout routines.
		 */
		...
		/*
		 * The style bits here must match the fsSelection bits in the OS/2 table, if present.
		 */
		...
		/*
		 * We don't need the remainder of data in this table: smallest readable size, font direction hint, indexToLocFormat, and glyphDataFormat.
		 */
	}
	...
	protected function _parseNameTable() {
		...
		/*
		 * The name table begins with a short header, followed by each of the fixed-length name records, followed by the variable-length strings.
		 */
		...
		/*
		 * Scan the name records for those we're interested in. We'll skip over encodings and languages we don't understand or support. Prefer the Microsoft Unicode encoding for a given name/language combination, but use Mac Roman if nothing else is available. We will extract the actual strings later.
		 */
		$nameRecords = array ();
		for($nameIndex = 0; $nameIndex < $nameCount; $nameIndex ++) {
			...
			null) {
                ...
                continue// Not a supported encoding. Move on.
				null;
			}
			...
			/*
			 * Entries in the name table are sorted by platform ID. If an entry exists for both Mac Roman and Microsoft Unicode, the Unicode entry will prevail since it is processed last.
			 */
			$nameRecords [$nameID] [$languageCode] = array (
					'platform' => $platformID,
					'offset' => $nameOffset,
					'length' => $nameLength 
			);
	...
	protected function _parsePostTable() {
		...
		/*
		 * The format of the remainder of this table is dependent on the table version. However, since it contains glyph ordering information and PostScript names which we don't use, move on. (This may change at some point in the future though...)
		 */
	}
	...
	protected function _parseHheaTable() {
		...
		/*
		 * The typographic ascent, descent, and line gap values are Apple- specific. Similar values exist in the OS/2 table. We'll use these values unless better values are found in OS/2.
		 */
		...
		/*
		 * The descent value is supposed to be negative--it's the distance relative to the baseline. However, some fonts improperly store a positive value in this field. If a positive value is found, flip the sign and record a warning in the debug log that we did this.
		 */
		if ($this->descent > 0) {
		...
		/*
		 * Skip over advance width, left and right sidebearing, max x extent, caret slope rise, run, and offset, and the four reserved fields.
		 */
		$this->skipBytes ( 22 );
	...
	protected function _parseOs2Table() {
		...
		try {
			...
			if ($e->getCode () == Zend_Pdf_Exception::REQUIRED_TABLE_NOT_FOUND) {
                ...
                $this// the style of the font is unknown
				null->isSansSerifFont = false;
		...
		/*
		 * Indicates the visual weight and aspect ratio of the characters. Used primarily to logically sort fonts in lists. Also used to help choose a more appropriate substitute font when necessary. See the WEIGHT_ and WIDTH_ constants defined in Zend_Pdf_Font.
		 */
		$this->fontWeight = $this->readUInt ( 2 );
		$this->fontWidth = $this->readUInt ( 2 );
		
		/*
		 * Describes the font embedding licensing rights. We can only embed and subset a font when given explicit permission. NOTE: We always interpret these bits according to the rules defined in version 3 of this table, regardless of the actual version. This means we will perform our checks in order from the most-restrictive to the least.
		 */
		$embeddingFlags = $this->readUInt ( 2 );
		$this->_debugLog ( 'Embedding flags: %d', $embeddingFlags );
		if ($this->isBitSet ( 9, $embeddingFlags )) {
			/*
			 * Only bitmaps may be embedded. We don't have the ability to strip outlines from fonts yet, so this means no embed.
			 */
			$this->isEmbeddable = false;
		} else if ($this->isBitSet ( 1, $embeddingFlags )) {
			/*
			 * Restricted license embedding. We currently don't have any way to enforce this, so interpret this as no embed. This may be revised in the future...
			 */
			$this->isEmbeddable = false;
		} else {
			/*
			 * The remainder of the bit settings grant us permission to embed the font. There may be additional usage rights granted or denied but those only affect the PDF viewer application, not our code.
			 */
			$this->isEmbeddable = true;
		}
		...
		/*
		 * Describes the class of font: serif, sans serif, script. etc. These values are defined here: http://www.microsoft.com/OpenType/OTSpec/ibmfc.htm
		 */
		$familyClass = ($this->readUInt ( 2 ) >> 8); // don't care about subclass
        $this// don't care about subclass
		null->_debugLog ( 'Font family class: %d', $familyClass );
		$this->isSerifFont = ((($familyClass >= 1) && ($familyClass <= 5)) || ($familyClass == 7));
		...
		/*
		 * Skip over the PANOSE number. The interesting values for us overlap with the font family class defined above.
		 */
		$this->skipBytes ( 10 );
		
		/*
		 * The Unicode range is made up of four 4-byte unsigned long integers which are used as bitfields covering a 128-bit range. Each bit represents a Unicode code block. If the bit is set, this font at least partially covers the characters in that block.
		 */
		 ...
		 */
		$this->isAdobeLatinSubset = (($unicodeRange1 == 1) && ($unicodeRange2 == 0) && ($unicodeRange3 == 0) && ($unicodeRange4 == 0));
		$this->_debugLog ( ($this->isAdobeLatinSubset ? 'Is' : 'Is not') . ' a subset of Adobe Latin' );
		
		$this->vendorID = $this->readBytes ( 4 );
		
		/*
		 * Skip the font style bits. We use the values found in the 'head' table. Also skip the first Unicode and last Unicode character indicies. Our cmap implementation does not need these values.
		 */
		$this->skipBytes ( 6 );
		
		/*
		 * Typographic ascender, descender, and line gap. These values are preferred to those in the 'hhea' table.
		 */
		$this->ascent = $this->readInt ( 2 );
		$this->descent = $this->readInt ( 2 );
		$this->lineGap = $this->readInt ( 2 );
		...
		/*
		 * Versions 0 and 1 tables do not contain the x or capital height fields. Record zero for unknown.
		 */
		if ($tableVersion < 2) {
			$this->xHeight = 0;
			$this->capitalHeight = 0;
			...
			/*
			 * Skip over the Windows code page coverages. We are only concerned with Unicode coverage.
			 */
			$this->skipBytes ( 8 );
	...
	protected function _parseHmtxTable() {
		...
		/*
		 * The hmtx table has no header. For each glpyh in the font, it contains the glyph's advance width and its left side bearing. We don't use the left side bearing.
		 */
		$glyphWidths = array ();
		...
		/*
		 * There is an optional table of left side bearings which is sometimes used for monospaced fonts. We don't use the left side bearing, so we can safely ignore it.
		 */
	}
	...
	protected function _parseCmapTable() {
		...
		/*
		 * Like the name table, there may be many different encoding subtables present. Ideally, we are looking for an acceptable Unicode table.
		 */
		$subtables = array ();
		for($subtableIndex = 0; $subtableIndex < $subtableCount; $subtableIndex ++) {
			...
			null(($platformID == 0) && ($encodingID == 0)) || 			// Unicode
                    // Unicode
			null(($platformID == 3) && ($encodingID == 1)) || 			// Microsoft Unicode
                    // Microsoft Unicode
			null(($platformID == 1) && ($encodingID == 0)))			// Mac Roman
                   )// Mac Roman
			null) {
			...
			if ($subtableOffset < 0) { // Sanity check for 4-byte unsigned on 32-bit platform
                $this// Sanity check for 4-byte unsigned on 32-bit platform
				null->_debugLog ( 'Offset 0x%x out of range for platformID: %d; skipping', $subtableOffset, $platformID );
				continue;
			}
		...
		foreach ( $offsets as $offset ) {
			...
			switch ($format) {
                ...
                case// break intentionally omitted
				null 0x8 : // break intentionally omitted
                case// break intentionally omitted
				null 0xa : // break intentionally omitted
                case// break intentionally omitted
				null 0xc :
	...
	protected function _readScalerType() {
		...
		switch ($this->_scalerType) {
			case 0x00010000 : // version 1.0 - Windows TrueType signature
                $this// version 1.0 - Windows TrueType signature
				null->_debugLog ( 'Windows TrueType signature' );
				break;
			
			case 0x74727565 : // 'true' - Macintosh TrueType signature
                $this// 'true' - Macintosh TrueType signature
				null->_debugLog ( 'Macintosh TrueType signature' );
				break;
			
			case 0x4f54544f : // 'OTTO' - the CFF signature
                $this// 'OTTO' - the CFF signature
				null->_debugLog ( 'PostScript CFF signature' );
				break;
			
			case 0x74797031 : // 'typ1'
                require_once// 'typ1'
				null 'Zend/Pdf/Exception.php';
	...
	protected function _jumpToTable($tableName) {
		if (empty ( $this->_tableDirectory [$tableName] )) { // do not allow NULL or zero
            require_once// do not allow NULL or zero
			null 'Zend/Pdf/Exception.php';
	 ...
	 * @param float $maxVertion
	 *        	Maximum compatible version number.
	 ...
	 * TrueType platform and language ID.
	 * Returns NULL for languages that are
	 * not supported.
	 *
	...
	protected function _languageCodeForPlatform($platformID, $languageID) {
		if ($platformID == 3) { // Microsoft encoding.
			/*
			 * The low-order bytes specify the language, the high-order bytes specify the dialect. We just care about the language. For the complete list, see: http://www.microsoft.com/globaldev/reference/lcid-all.mspx
			 */
			$languageID &= 0xff;
			switch ($languageID) {
				case 0x09 :
