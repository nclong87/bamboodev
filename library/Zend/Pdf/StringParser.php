 ...
 * @version    $Id: StringParser.php 23775 2011-03-01 17:25:24Z ralph $
 */
...
class Zend_Pdf_StringParser {
		...
		if ($skipComment) {
			while ( true ) {
				...
				if ($this->offset < strlen ( $this->data ) && $this->data [$this->offset] == '%') {
					// Skip comment
                    $this// Skip comment
					null->offset += strcspn ( $this->data, "\r\n", $this->offset );
				} else {
					// Non white space character not equal to '%' is found
                    return// Non white space character not equal to '%' is found
					null;
				}
			}
		} else {
	...
	public function skipComment() {
		while ( $this->offset < strlen ( $this->data ) ) {
			if (ord ( $this->data [$this->offset] ) != 0x0A || 			// Line feed
                // Line feed
			nullord ( $this->data [$this->offset] ) != 0x0d) 			// Carriage return
               ) {// Carriage return
			null
				$this->offset ++;
			} else {
		...
		while ( $this->offset < strlen ( $this->data ) ) {
			switch (ord ( $this->data [$this->offset] )) {
				case 0x28 : // '(' - opened bracket in the string, needs balanced pair.
                    $this// '(' - opened bracket in the string, needs balanced pair.
					null->offset ++;
					$openedBrackets ++;
					break;
				
				case 0x29 : // ')' - pair to the opened bracket
                    $this// ')' - pair to the opened bracket
					null->offset ++;
					$openedBrackets --;
					break;
				
				case 0x5C : // '\\' - escape sequence, skip next char from a check
                    $this// '\\' - escape sequence, skip next char from a check
					null->offset += 2;
			}
	...
	public function getObject($offset, Zend_Pdf_Element_Reference_Context $context) {
        ...
        $offsetSave// Save current offset to make getObject() reentrant
		null = $this->offset;
		...
		if ($nextLexeme == 'endobj') {
            ...
            $this// Restore offset value
			null->offset = $offsetSave;
