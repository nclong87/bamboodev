...
class Zend_Date extends Zend_Date_DateObject {
    ...
    )// timesync server to set
	null;
	
	// Class wide Date Constants
    const// Class wide Date Constants
	null DAY = 'dd';
	const DAY_SHORT = 'd';
	const DAY_SUFFIX = 'SS';
	const DAY_OF_YEAR = 'D';
	 ...
	 * string, integer, array and so on.
	 * Also parts of dates or time are supported
	...
	public function __construct($date = null, $part = null, $locale = null) {
		if (is_object ( $date ) and ! ($date instanceof Zend_TimeSync_Protocol) and ! ($date instanceof Zend_Date)) {
		...
		if (($date !== null) and ! is_array ( $date ) and ! ($date instanceof Zend_TimeSync_Protocol) and ! ($date instanceof Zend_Date) and ! defined ( $date ) and Zend_Locale::isLocale ( $date, true, false )) {
			$locale = $date;
			$date = null;
			$part = null;
        ...
        $zone// set the timezone and offset for $this
		null = @date_default_timezone_get ();
		$this->setTimezone ( $zone );
		
		// try to get timezone from date-string
        if// try to get timezone from date-string
		null (! is_int ( $date )) {
		...
		// set datepart
        if// set datepart
		null (($part !== null && $part !== self::TIMESTAMP) or (! is_numeric ( $date ))) {
			// switch off dst handling for value setting
            $this// switch off dst handling for value setting
			null->setUnixTimestamp ( $this->getGmtOffset () );
			$this->set ( $date, $part, $this->_locale );
			
			// DST fix
            if// DST fix
			null (is_array ( $date ) === true) {
				if (! isset ( $date ['hour'] )) {
					$date ['hour'] = 0;
				}
	 ...
	 * @param string|integer|array|Zend_Date $stamp
	...
	private function _timestamp($calc, $stamp) {
		if ($stamp instanceof Zend_Date) {
            ...
            $stamp// extract timestamp from object
			null = $stamp->getTimestamp ();
		}
	 ...
	 *
	 * @param integer|string|array|Zend_Date $timestamp
	 *        	Timestamp to set
	...
	public function toString($format = null, $type = null, $locale = null) {
		...
		if (($format !== null) && ! defined ( $format ) && ($format != 'ee') && ($format != 'ss') && ($format != 'GG') && ($format != 'MM') && ($format != 'EE') && ($format != 'TT') && Zend_Locale::isLocale ( $format, null, false )) {
			$locale = $format;
			$format = null;
		}
		
		if (($type !== null) and ($type != 'php') and ($type != 'iso') and Zend_Locale::isLocale ( $type, null, false )) {
			$locale = $type;
			$type = null;
		}
	 ...
	 * But returns false when the given part is no value f.e.
	 * Month-Name
	 *
	 * @param string|integer|Zend_Date $part
	 ...
	 * the era or only the fractional seconds.
	...
	public function get($part = null, $locale = null) {
		...
		if (($part !== null) && ! defined ( $part ) && ($part != 'ee') && ($part != 'ss') && ($part != 'GG') && ($part != 'MM') && ($part != 'EE') && ($part != 'TT') && Zend_Locale::isLocale ( $part, null, false )) {
			$locale = $part;
			$part = null;
		}
	...
	private function _parseIsoToDate($token, $locale) {
		switch ($token) {
			...
			case self::WEEKDAY_SHORT :
				...
				$day = Zend_Locale_Data::getContent ( $locale, 'day', array (
						'gregorian',
						'format',
						'wide',
						$weekday 
			...
			case self::WEEKDAY :
				$weekday = strtolower ( $this->date ( 'D', $this->getUnixTimestamp (), false ) );
				return $this->_toComment ( Zend_Locale_Data::getContent ( $locale, 'day', array (
						'gregorian',
						'format',
						'wide',
						$weekday 
			...
			case self::MONTH_NAME :
				...
				return $this->_toComment ( Zend_Locale_Data::getContent ( $locale, 'month', array (
						'gregorian',
						'format',
						'wide',
						$month 
			...
			case self::MONTH_NAME_NARROW :
				$month = $this->date ( 'n', $this->getUnixTimestamp (), false );
				$mon = Zend_Locale_Data::getContent ( $locale, 'month', array (
						'gregorian',
						'format',
						'abbreviated',
						$month 
			...
			case 'GGG' :
				...
				if ($year < 0) {
					return $this->_toComment ( Zend_Locale_Data::getContent ( $locale, 'era', array (
							'gregorian',
							'Abbr',
							'0' 
					) ) );
			...
			case self::ERA_NARROW :
				$year = $this->date ( 'Y', $this->getUnixTimestamp (), false );
				if ($year < 0) {
					return $this->_toComment ( iconv_substr ( Zend_Locale_Data::getContent ( $locale, 'era', array (
							'gregorian',
							'Abbr',
							'0' 
					) ), 0, 1, 'UTF-8' ) ) . '.';
			...
			case self::DATE_FULL :
				return $this->_toToken ( Zend_Locale_Data::getContent ( $locale, 'date', array (
						'gregorian',
						'full' 
				) ), $locale );
				break;
			...
			case self::DATETIME_FULL :
				return $this->_toToken ( Zend_Locale_Data::getContent ( $locale, 'datetime', array (
						'gregorian',
						'full' 
				) ), $locale );
				break;
	 ...
	 * the month or only the seconds.
	 * There are about 50 different supported date parts.
	 * For a complete list of supported datepart values look into the docu
	 *
	 * @param string|integer|array|Zend_Date $date
	 ...
	 * and modifying this object by adding that part.
	 * The $part is then extracted from
	...
	public function copyPart($part, $locale = null) {
        ...
        $clone// copy all instance variables
		null->setUnixTimestamp ( 0 ); // except the timestamp
        if// except the timestamp
		null ($locale != null) {
			$clone->setLocale ( $locale ); // set an other locale if selected
        }// set an other locale if selected
		null
		$clone->set ( $this, $part );
		return $clone;
	}
	 ...
	 * @param string|integer $comp
	 *        	Second date for calculation
	 * @param boolean|integer $dst
	 *        	Use dst correction if option is set
	 * @return integer string Zend_Date or Zend_Date depending on calculation
	 */
	private function _assign($calc, $date, $comp = 0, $dst = false) {
		switch ($calc) {
			...
			default :
				// cmp - compare
                return// cmp - compare
				null call_user_func ( Zend_Locale_Math::$comp, $comp, $date );
				break;
		}
		
		// dst-correction if 'fix_dst' = true and dst !== false but only for non UTC and non GMT
        if// dst-correction if 'fix_dst' = true and dst !== false but only for non UTC and non GMT
		null ((self::$_options ['fix_dst'] === true) and ($dst !== false) and ($this->_dst === true)) {
	 ...
	 * @return integer string Zend_Date
	 * @throws Zend_Date_Exception
	 */
	private function _calculate($calc, $date, $part, $locale) {
		...
		// Create date parts
        $year// Create date parts
		null = $this->toString ( self::YEAR, 'iso' );
		$month = $this->toString ( self::MONTH_SHORT, 'iso' );
		$day = $this->toString ( self::DAY_SHORT, 'iso' );
		$hour = $this->toString ( self::HOUR_SHORT, 'iso' );
		$minute = $this->toString ( self::MINUTE_SHORT, 'iso' );
        ...
        if// If object extract value
		null ($date instanceof Zend_Date) {
        ...
        switch// $date as object, part of foreign date as own date
		null ($part) {
			...
			case self::WEEKDAY_SHORT :
				...
				// Weekday not found
                require_once// Weekday not found
				null 'Zend/Date/Exception.php';
			...
			case self::WEEKDAY_NARROW :
				$daylist = Zend_Locale_Data::getList ( $locale, 'day', array (
						'gregorian',
						'format',
						'abbreviated' 
				) );
			...
			null self::MONTH_NAME :
				...
				// Monthname not found
                require_once// Monthname not found
				null 'Zend/Date/Exception.php';
			...
			case self::MONTH_NAME_SHORT :
				$monthlist = Zend_Locale_Data::getList ( $locale, 'month', array (
						'gregorian',
						'format',
						'abbreviated' 
				) );
			...
			null self::ISO_8601 :
				// (-)YYYY-MM-dd
                // (-)YYYY-MM-dd
				nullpreg_match ( '/^(-{0,1}\d{4})-(\d{2})-(\d{2})/', $date, $datematch );
				// (-)YY-MM-dd
                if// (-)YY-MM-dd
				null (empty ( $datematch )) {
				...
				// (T)hh:mm:ss
                // (T)hh:mm:ss
				nullpreg_match ( '/[T,\s]{0,1}(\d{2}):(\d{2}):(\d{2})/', $tmpdate, $timematch );
				if (empty ( $timematch )) {
			...
			case self::DATES :
				try {
					$parsed = Zend_Locale_Format::getDate ( $date, array (
							'locale' => $locale,
							'format_type' => 'iso',
							'fix_date' => true 
					) );
			...
			case self::DATE_FULL :
				try {
					$format = Zend_Locale_Data::getContent ( $locale, 'date', array (
							'gregorian',
							'full' 
					) );
					$parsed = Zend_Locale_Format::getDate ( $date, array (
			...
			case self::TIMES :
				try {
					...
					$parsed = Zend_Locale_Format::getTime ( $date, array (
							'locale' => $locale,
							'format_type' => 'iso',
							'fix_date' => true 
					) );
			...
			case self::TIME_FULL :
				try {
					$format = Zend_Locale_Data::getContent ( $locale, 'time', array (
							'gregorian',
							'full' 
					) );
					$parsed = Zend_Locale_Format::getTime ( $date, array (
			...
			case self::DATETIME :
				try {
					$parsed = Zend_Locale_Format::getDateTime ( $date, array (
							'locale' => $locale,
							'format_type' => 'iso',
							'fix_date' => true 
					) );
			...
			case self::DATETIME_FULL :
				try {
					$format = Zend_Locale_Data::getContent ( $locale, 'datetime', array (
							'gregorian',
							'full' 
					) );
					$parsed = Zend_Locale_Format::getDateTime ( $date, array (
            ...
            case// ATOM and RFC_3339 are identical
			null self::ATOM :
			case self::RFC_3339 :
			...
			case self::RFC_1036 :
                ...
                $result// new RFC 822 format, identical to RFC 1036 standard
				null = preg_match ( '/^\w{0,3},{0,1}\s{0,1}(\d{1,2})\s(\w{3})\s(\d{2})\s(\d{2}):(\d{2}):{0,1}(\d{0,2})\s([+-]{1}\d{4}|\w{1,20})$/', $date, $match );
				if (! $result) {
					require_once 'Zend/Date/Exception.php';
			...
			default :
				if (! is_numeric ( $date ) || ! empty ( $part )) {
					try {
						...
						return $this->_assign ( $calc, $this->mktime ( isset ( $parsed ['hour'] ) ? $parsed ['hour'] : 0, isset ( $parsed ['minute'] ) ? $parsed ['minute'] : 0, isset ( $parsed ['second'] ) ? $parsed ['second'] : 0, isset ( $parsed ['month'] ) ? (1 + $parsed ['month']) : 1, isset ( $parsed ['day'] ) ? (1 + $parsed ['day']) : 1, isset ( $parsed ['year'] ) ? (1970 + $parsed ['year']) : 1970, false ), $this->getUnixTimestamp (), false );
					} catch ( Zend_Locale_Exception $e ) {
						if (! is_numeric ( $date )) {
							require_once 'Zend/Date/Exception.php';
							throw new Zend_Date_Exception ( $e->getMessage (), 0, $e, $date );
	...
	private function _time($calc, $time, $format, $locale) {
		...
		if ($time instanceof Zend_Date) {
            ...
            $time// extract time from object
			null = $time->toString ( 'HH:mm:ss', 'iso' );
		} else {
			if (is_array ( $time )) {
				...
				try {
					...
					$parsed = Zend_Locale_Format::getTime ( $time, array (
							'date_format' => $format,
							'locale' => $locale,
							'format_type' => 'iso' 
					) );
	 ...
	 * Format defines how to parse the time string.
	...
	private function _date($calc, $date, $format, $locale) {
		...
		if ($date instanceof Zend_Date) {
            ...
            $date// extract date from object
			null = $date->toString ( 'd.M.y', 'iso' );
		} else {
			if (is_array ( $date )) {
				if ((isset ( $date ['year'] ) === true) or (isset ( $date ['month'] ) === true) or (isset ( $date ['day'] ) === true)) {
					$parsed = $date;
				} else {
					require_once 'Zend/Date/Exception.php';
	...
	/**
	 * Sets a new date for the date object.
	 * Not given parts are set to default.
	 ...
	 *
	 * @param string|integer|Zend_Date $date
	 *        	ISO Date to set
	 * @param string|Zend_Locale $locale
	 *        	OPTIONAL Locale for parsing input
	 * @return Zend_Date Provides fluid interface
	 ...
	 *
	 * @param integer|array|Zend_Date $year
	 *        	Year to check
	 * @return boolean
	 */
	...
	public function isYesterday() {
        ...
        $yesterday// adjusts for leap days and DST changes that are timezone specific
		null = $this->date ( 'Ymd', $this->mktime ( 0, 0, 0, $month, $day - 1, $year ) );
		$day = $this->date ( 'Ymd', $this->getUnixTimestamp () );
	...
	public function isTomorrow() {
		list ( $year, $month, $day ) = explode ( '-', $this->date ( 'Y-m-d', $this->_getTime () ) );
		// adjusts for leap days and DST changes that are timezone specific
        $tomorrow// adjusts for leap days and DST changes that are timezone specific
		null = $this->date ( 'Ymd', $this->mktime ( 0, 0, 0, $month, $day + 1, $year ) );
		$day = $this->date ( 'Ymd', $this->getUnixTimestamp () );
		return $day == $tomorrow;
	}
	 ...
	 * @param string $calc
	 *        	Calculation to make
	 * @param string|integer|Zend_Date $value
	...
	private function _calcvalue($calc, $value, $type, $parameter, $locale) {
		...
		if ($value instanceof Zend_Date) {
            ...
            $value// extract value from object
			null = $value->toString ( $parameter, 'iso', $locale );
	 ...
	 * 3 or 4 digit years are set as expected.
	 * If you need to set year 0-99
	 * use set() instead.
	 * Returned is the new date object
	 *
	 * @param string|integer|array|Zend_Date $date
	 ...
	 * 3 or 4 digit years are added as expected.
	 * If you need to add years from 0-99
	 * use add() instead.
	 * Returned is the new date object
	 *
	 * @param string|integer|array|Zend_Date $date
	 ...
	 * 3 or 4 digit years are subtracted as expected.
	 * If you need to subtract years from 0-99
	 * use sub() instead.
	 * Returned is the new date object
	 *
	 * @param string|integer|array|Zend_Date $date
	 ...
	 * Returns if equal, earlier or later
	 *
	 * @param string|integer|array|Zend_Date $year
	 *        	Year to compare
	 * @param string|Zend_Locale $locale
	 *        	OPTIONAL Locale for parsing input
	 * @return integer 0 = equal, 1 = later, -1 = earlier
	 ...
	 * @param string $calc
	 *        	Calculation to make
	 * @param string|integer|array|Zend_Date $month
	...
	private function _month($calc, $month, $locale) {
		...
		if ($month instanceof Zend_Date) {
            ...
            $found// extract month from object
			null = $month->toString ( self::MONTH_SHORT, 'iso', $locale );
		} else {
			if (is_numeric ( $month )) {
				$found = $month;
				...
				$monthlist = Zend_Locale_Data::getList ( $locale, 'month' );
				$monthlist2 = Zend_Locale_Data::getList ( $locale, 'month', array (
						'gregorian',
						'format',
						'abbreviated' 
				) );
	...
	/**
	 * Sets a new month
	 * The month can be a number or a string.
	 * Setting months lower then 0 and greater then 12
	...
	/**
	 * Sets a new day
	 * The day can be a number or a string.
	 * Setting days lower then 0 or greater than the number of this months days
	 ...
	 * Returns if equal, earlier or later
	 *
	 * @param string|integer|array|Zend_Date $day
	 *        	Day to compare
	 * @param string|Zend_Locale $locale
	 *        	OPTIONAL Locale for parsing input
	 * @return integer 0 = equal, 1 = later, -1 = earlier
	 ...
	 * If a localized weekday name is given,
	 ...
	 * Returns if equal, earlier or later
	 *
	 * @param string|integer|array|Zend_Date $weekday
	 *        	Weekday to compare
	 * @param string|Zend_Locale $locale
	 *        	OPTIONAL Locale for parsing input
	 * @return integer 0 = equal, 1 = later, -1 = earlier
	 ...
	 *
	 * @param string|integer|array|Zend_Date $hour
	 *        	Hour to set
	 * @param string|Zend_Locale $locale
	 *        	OPTIONAL Locale for parsing input
	 * @return Zend_Date Provides fluid interface
	 ...
	 *
	 * @param string|integer|array|Zend_Date $second
	 *        	Second to set
	 * @param string|Zend_Locale $locale
	 *        	(Optional) Locale for parsing input
	 * @return Zend_Date Provides fluid interface
	...
	public function addMilliSecond($milli = null, $precision = null) {
        ...
        $max// Add/sub milliseconds + add/sub seconds
		null = pow ( 10, $this->_precision );
		// Milli includes seconds
        if// Milli includes seconds
		null ($this->_fractional >= $max) {
	...
	/**
	 * Sets a new week.
	 * The week is always a number. The day of week is not changed.
	 * Returned is the new date object
	 * Example: 09.Jan.2007 13:07:25 -> setWeek(1); -> 02.Jan.2007 13:07:25
	 *
	 * @param string|integer|array|Zend_Date $week
	 *        	Week to set
	 * @param string|Zend_Locale $locale
	 *        	OPTIONAL Locale for parsing input
	 * @return Zend_Date Provides fluid interface
	 ...
	 *
	 * @param string|array|Zend_Date $date
	...
	protected static function _getLocalizedToken($token, $locale) {
		switch ($token) {
			...
			case self::DATES :
				...
				break;
			case self::DATE_FULL :
				return Zend_Locale_Data::getContent ( $locale, 'date', array (
						'gregorian',
						'full' 
				) );
				break;
			...
			case self::TIMES :
				...
				break;
			case self::TIME_FULL :
				return Zend_Locale_Data::getContent ( $locale, 'time', array (
						'gregorian',
						'full' 
				) );
				break;
			...
			case self::DATETIME :
				...
				break;
			case self::DATETIME_FULL :
				return Zend_Locale_Data::getContent ( $locale, 'datetime', array (
						'gregorian',
						'full' 
				) );
				break;
