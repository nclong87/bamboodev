...
class Zend_OpenId {
	 ...
	 * All OpenID 2.0 messages MUST contain variable
	...
	static public function normalizeUrl(&$id) {
		...
		while ( $i < $n ) {
			if ($id [$i] == '%') {
				...
				++ $i;
				$ch = chr ( $c );
				if (($ch >= 'A' && $ch <= 'Z') || ($ch >= 'a' && $ch <= 'z') || $ch == '-' || $ch == '.' || $ch == '_' || $ch == '~') {
					$res .= $ch;
				} else {
					$res .= '%';
					if (($c >> 4) < 10) {
		...
		$id = $scheme . '://' . $auth . $host . (empty ( $port ) ? '' : (':' . $port)) . $path . $query . $fragment;
		return true;
	}
	 ...
	 */
	static public function redirect($url, $params = null, Zend_Controller_Response_Abstract $response = null, $method = 'GET') {
	 ...
	 * generator $g.
	 * Both sides of key exchange MUST have the same prime number
	 ...
	 * The array includes original prime number 'p' and
