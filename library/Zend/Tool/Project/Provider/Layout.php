...
class Zend_Tool_Project_Provider_Layout extends Zend_Tool_Project_Provider_Abstract implements Zend_Tool_Framework_Provider_Pretendable {
	...
	public static function createResource(Zend_Tool_Project_Profile $profile, $layoutName = 'layout') {
		...
		$layoutScriptFile = $layoutScriptsDirectory->search ( 'layoutScriptFile', array (
				'layoutName' => 'layout' 
		) );
		
		if ($layoutScriptFile == false) {
			$layoutScriptFile = $layoutScriptsDirectory->createResource ( 'layoutScriptFile', array (
					'layoutName' => 'layout' 
			) );
		}
	...
	public function enable() {
		...
		if ($this->_registry->getRequest ()->isPretend ()) {
			...
			if (! $layoutScriptFile->exists ()) {
				...
				$this->_registry->getResponse ()->appendContent ( 'A default layout has been created at ' . $layoutScriptFile->getPath () );
			}
