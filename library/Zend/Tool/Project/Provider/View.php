...
class Zend_Tool_Project_Provider_View extends Zend_Tool_Project_Provider_Abstract {
	...
	public static function createResource(Zend_Tool_Project_Profile $profile, $actionName, $controllerName, $moduleName = null) {
		...
		$profileSearchParams ['viewControllerScriptsDirectory'] = array (
		...
		$newViewScriptFile = $viewControllerScriptsDirectory->createResource ( 'ViewScriptFile', array (
	...
	public function create($controllerName, $actionNameOrSimpleName, $module = null) {
		...
		if ($this->_registry->getRequest ()->isPretend ()) {
			$this->_registry->getResponse ( 'Would create a view script in location ' . $view->getContext ()->getPath () );
		} else {
			$this->_registry->getResponse ( 'Creating a view script in location ' . $view->getContext ()->getPath () );
			$view->create ();
			$this->_storeProfile ();
		}
	}
