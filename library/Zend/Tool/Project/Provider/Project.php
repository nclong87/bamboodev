 ...
 * @license http://framework.zend.com/license/new-bsd New BSD License
 */
class Zend_Tool_Project_Provider_Project extends Zend_Tool_Project_Provider_Abstract// implements Zend_Tool_Framework_Provider_DocblockManifestInterface
{
	 ...
	 *
	 * @param string $path        	
	 * @param string $nameOfProfile
	 *        	shortName=n
	 * @param string $fileOfProfile
	 *        	shortName=f
	 */
	public function create($path, $nameOfProfile = null, $fileOfProfile = null) {
		...
		$response->appendContent ( 'Note: ', array (
				'separator' => false,
				'color' => 'yellow' 
		) );
		...
		if (! Zend_Tool_Project_Provider_Test::isPHPUnitAvailable ()) {
			$response->appendContent ( 'Testing Note: ', array (
					'separator' => false,
					'color' => 'yellow' 
			) );
	...
	public function show() {
		$this->_registry->getResponse ()->appendContent ( 'You probably meant to run "show project.info".', array (
				'color' => 'yellow' 
		) );
	}
