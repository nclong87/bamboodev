...
abstract class Zend_Search_Lucene_Storage_File {
	...
	public function writeInt($value) {
		...
		$this->_fwrite ( chr ( $value >> 24 & 0xFF ) . chr ( $value >> 16 & 0xFF ) . chr ( $value >> 8 & 0xFF ) . chr ( $value & 0xFF ), 4 );
	}
	...
	public function readLong() {
		...
		if (PHP_INT_SIZE > 4) {
			...
			return ord ( $str [0] ) << 56 | ord ( $str [1] ) << 48 | ord ( $str [2] ) << 40 | ord ( $str [3] ) << 32 | ord ( $str [4] ) << 24 | ord ( $str [5] ) << 16 | ord ( $str [6] ) << 8 | ord ( $str [7] );
		} else {
	...
	public function writeLong($value) {
		...
		if (PHP_INT_SIZE > 4) {
			settype ( $value, 'integer' );
			$this->_fwrite ( chr ( $value >> 56 & 0xFF ) . chr ( $value >> 48 & 0xFF ) . chr ( $value >> 40 & 0xFF ) . chr ( $value >> 32 & 0xFF ) . chr ( $value >> 24 & 0xFF ) . chr ( $value >> 16 & 0xFF ) . chr ( $value >> 8 & 0xFF ) . chr ( $value & 0xFF ), 8 );
		} else {
		...
		if ($strlen == 0) {
			...
			for($count = 0; $count < $strlen; $count ++) {
				if ((ord ( $str_val [$count] ) & 0xC0) == 0xC0) {
					...
					// in two bytes.
					if (ord ( $str_val [$count] ) == 0xC0 && ord ( $str_val [$count + 1] ) == 0x80) {
						$str_val [$count] = 0;
						$str_val = substr ( $str_val, 0, $count + 1 ) . substr ( $str_val, $count + 2 );
					}
					$count += $addBytes;
				}
			}
