	...
	public function encode() {
		if ($this->getFilename () !== null && is_readable ( $this->getFilename () )) {
			...
			if ($result === false) {
				...
				throw new Zend_Gdata_App_IOException ( "Error reading file - " . $this->getFilename () . '. Read failed.' );
			}
			fclose ( $fileHandle );
			return $result;
		} else {
			require_once 'Zend/Gdata/App/IOException.php';
			throw new Zend_Gdata_App_IOException ( "Error reading file - " . $this->getFilename () . '. File is not readable.' );
		}
	}
