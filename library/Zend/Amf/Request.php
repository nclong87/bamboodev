...
class Zend_Amf_Request {
	...
	public function readMessage(Zend_Amf_Parse_InputStream $stream) {
		...
		if (($clientVersion != Zend_Amf_Constants::AMF0_OBJECT_ENCODING) && ($clientVersion != Zend_Amf_Constants::AMF3_OBJECT_ENCODING) && ($clientVersion != Zend_Amf_Constants::FMS_OBJECT_ENCODING)) {
	...
	public function readBody() {
		...
		if ($this->_deserializer->getObjectEncoding () == Zend_Amf_Constants::AMF3_OBJECT_ENCODING) {
			/*
			 * When and AMF3 message is sent to the server it is nested inside an AMF0 array called Content. The following code gets the object out of the content array and sets it as the message data.
			 */
