...
class Zend_Auth_Adapter_Ldap implements Zend_Auth_Adapter_Interface {
	...
	public function authenticate() {
		...
		if (! $password) {
			/*
			 * A password is required because some servers will treat an empty password as an anonymous bind.
			 */
		...
		/*
		 * Iterate through each server and try to authenticate the supplied credentials against it.
		 */
		foreach ( $this->_options as $name => $options ) {
			...
			try {
				...
				/*
				 * Fixes problem when authenticated user is not allowed to retrieve group-membership information or own account. This requires that the user specified with "username" and optionally "password" in the Zend_Ldap options is able to retrieve the required information.
				 */
				$requireRebind = false;
				...
				/*
				 * LDAP based authentication is notoriously difficult to diagnose. Therefore we bend over backwards to capture and record every possible bit of information when something goes wrong.
				 */
				...
				if ($err == Zend_Ldap_Exception::LDAP_X_DOMAIN_MISMATCH) {
					/*
					 * This error indicates that the domain supplied in the username did not match the domains in the server options and therefore we should just skip to the next set of server options.
					 */
					continue;
