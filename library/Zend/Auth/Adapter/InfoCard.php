...
class Zend_Auth_Adapter_InfoCard implements Zend_Auth_Adapter_Interface {
	...
	public function authenticate() {
		...
		if (! $claims->isValid ()) {
			switch ($claims->getCode ()) {
				case Zend_infoCard_Claims::RESULT_PROCESSING_FAILURE :
					return new Zend_Auth_Result ( Zend_Auth_Result::FAILURE, $claims, array (
							'Processing Failure',
							$claims->getErrorMsg () 
					) );
					break;
				case Zend_InfoCard_Claims::RESULT_VALIDATION_FAILURE :
					return new Zend_Auth_Result ( Zend_Auth_Result::FAILURE_CREDENTIAL_INVALID, $claims, array (
							'Validation Failure',
							$claims->getErrorMsg () 
					) );
					break;
		...
		return new Zend_Auth_Result ( Zend_Auth_Result::SUCCESS, $claims );
	}
}
