...
class Zend_View_Helper_Navigation_Sitemap extends Zend_View_Helper_Navigation_HelperAbstract {
	...
	public function setServerUrl($serverUrl) {
		...
		if ($uri->valid ()) {
			...
			$e = new Zend_Uri_Exception ( sprintf ( 'Invalid server URL: "%s"', $serverUrl ) );
			$e->setView ( $this->view );
			throw $e;
		}
	...
	public function getDomSitemap(Zend_Navigation_Container $container = null) {
		...
		if ($this->getUseSchemaValidation ()) {
			if (! @$dom->schemaValidate ( self::SITEMAP_XSD )) {
				require_once 'Zend/View/Exception.php';
				$e = new Zend_View_Exception ( sprintf ( 'Sitemap is invalid according to XML Schema at "%s"', self::SITEMAP_XSD ) );
				$e->setView ( $this->view );
				throw $e;
			}
		}
