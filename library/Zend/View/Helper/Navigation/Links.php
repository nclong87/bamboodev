...
class Zend_View_Helper_Navigation_Links extends Zend_View_Helper_Navigation_HelperAbstract {
	 ...
	 * @return Zend_Navigaiton_Page array null null if not found
	 * @throws Zend_View_Exception if $rel is not "rel" or "rev"
	 */
	public function findRelation(Zend_Navigation_Page $page, $rel, $type) {
		if (! in_array ( $rel, array (
				'rel',
				'rev' 
		) )) {
			require_once 'Zend/View/Exception.php';
			$e = new Zend_View_Exception ( sprintf ( 'Invalid argument: $rel must be "rel" or "rev"; "%s" given', $rel ) );
			$e->setView ( $this->view );
			throw $e;
		}
	 ...
	 * @param string $type
	 *        	link type, e.g. 'start', 'next'
	 * @return Zend_Navigation_Page array null null if not found
	 */
	 ...
	 * @return array null of pages, or null if not found
	 */
	 ...
	 * @return Zend_Navigation_Page null or null
	 */
	 ...
	 * @param Zend_Navigation_Page $page
	 *        	page to find relation for
	 * @return Zend_Navigation_Page array null null
	 */
	public function searchRelChapter(Zend_Navigation_Page $page) {
		...
		foreach ( $root as $chapter ) {
			...
			if ($chapter !== $page && ! in_array ( $chapter, $start ) && $this->accept ( $chapter )) {
				$found [] = $chapter;
			}
		}
	...
	public function searchRevSection(Zend_Navigation_Page $page) {
		...
		if ($parent = $page->getParent ()) {
			if ($parent instanceof Zend_Navigation_Page && $this->_findRoot ( $page )->hasPage ( $parent )) {
				$found = $parent;
			}
		}
	 ...
	 * @return Zend_Navigation_Page array null unable to convert
	 */
	...
	public function renderLink(Zend_Navigation_Page $page, $attrib, $relation) {
		if (! in_array ( $attrib, array (
			...
			require_once 'Zend/View/Exception.php';
			$e = new Zend_View_Exception ( sprintf ( 'Invalid relation attribute "%s", must be "rel" or "rev"', $attrib ) );
			$e->setView ( $this->view );
			throw $e;
		}
