...
class Zend_Validate_EmailAddress extends Zend_Validate_Abstract {
	...
	private function _isReserved($host) {
		...
		if (( int ) $octet [0] >= 224) {
			...
			foreach ( ( array ) $this->_invalidIp [$octet [0]] as $subnetData ) {
				...
				for($j = $i; $j < 4; $j ++) {
					if (( int ) $octet [$j] < $segmentData ['network'] [$j] || ( int ) $octet [$j] > $segmentData ['broadcast'] [$j]) {
						return false;
					}
				}
			}
	...
	private function _validateMXRecords() {
		...
		if (! $result) {
			...
			foreach ( $mxHosts as $hostname ) {
				...
				if (! $res && (checkdnsrr ( $hostname, "A" ) || checkdnsrr ( $hostname, "AAAA" ) || checkdnsrr ( $hostname, "A6" ))) {
					$validAddress = true;
					break;
				}
			}
	...
	private function _validateHostnamePart() {
		$hostname = $this->_options ['hostname']->setTranslator ( $this->getTranslator () )->isValid ( $this->_hostname );
		if (! $hostname) {
