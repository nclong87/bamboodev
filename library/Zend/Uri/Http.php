...
class Zend_Uri_Http extends Zend_Uri {
	...
	protected function __construct($scheme, $schemeSpecific = '') {
		...
		$this->_regex ['uric'] = '(?:' . $this->_regex ['escaped'] . '|[' . self::CHAR_ALNUM . self::CHAR_MARK . self::CHAR_RESERVED .
...
(self::$_config ['allow_unwise'] ? self::CHAR_UNWISE : '') . '])';
	...
	protected function _parseUri($schemeSpecific) {
		...
		$this->_host = isset ( $matches [5] ) === true ? preg_replace ( '~^\[([^]]+)\]$~', '\1', $matches [5] ) : 		// Strip wrapper [] from IPv6 literal
		'';
