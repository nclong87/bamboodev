...
class Zend_Rest_Route extends Zend_Controller_Router_Route_Module {
	...
	public function match($request, $partial = false) {
		...
		if ($path != '') {
			...
			if ($pathElementCount && array_search ( $path [0], array (
					'index',
					'new' 
			) ) > - 1) {
	...
	public function assemble($data = array(), $reset = false, $encode = true) {
		...
		if (in_array ( $params [$this->_actionKey], array (
				'new',
				'edit' 
		) )) {
	...
	protected function _checkRestfulController($moduleName, $controllerName) {
		...
		if ($this->_checkRestfulModule ( $moduleName ) && $this->_restfulControllers && (false !== array_search ( $controllerName, $this->_restfulControllers [$moduleName] ))) {
			return true;
		}
		return false;
	}
	...
	protected function _fullRestfulModule($moduleName) {
		return ($this->_restfulModules && (false !== array_search ( $moduleName, $this->_restfulModules )));
	}
}
