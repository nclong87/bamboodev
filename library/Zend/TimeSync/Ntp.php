...
class Zend_TimeSync_Ntp extends Zend_TimeSync_Protocol {
	...
	protected function _prepare() {
		...
		/*
		 * Root delay Indicates the total roundtrip delay to the primary reference source at the root of the synchronization subnet, in seconds
		 */
		$ntppacket .= $nul . $nul . chr ( 0x1c ) . chr ( 0x9b );
		
		/*
		 * Clock Dispersion Indicates the maximum error relative to the primary reference source at the root of the synchronization subnet, in seconds
		 */
		$ntppacket .= $nul . chr ( 0x08 ) . chr ( 0xd7 ) . chr ( 0xff );
		
		/*
		 * ReferenceClockID Identifying the particular reference clock
		 */
		$ntppacket .= $nulbyte;
		
		/*
		 * The local time, in timestamp format, at the peer when its latest NTP message was sent. Contanis an integer and a fractional part
		 */
		$ntppacket .= chr ( $secba ) . chr ( $secbb ) . chr ( $secbc ) . chr ( $secbd );
		$ntppacket .= chr ( $fracba ) . chr ( $fracbb ) . chr ( $fracbc ) . chr ( $fracbd );
		
		/*
		 * The local time, in timestamp format, at the peer. Contains an integer and a fractional part.
		 */
		$ntppacket .= $nulbyte;
		$ntppacket .= $nulbyte;
		
		/*
		 * This is the local time, in timestamp format, when the latest NTP message from the peer arrived. Contanis an integer and a fractional part.
		 */
		$ntppacket .= $nulbyte;
		$ntppacket .= $nulbyte;
		
		/*
		 * The local time, in timestamp format, at which the NTP message departed the sender. Contanis an integer and a fractional part.
		 */
		$ntppacket .= chr ( $secba ) . chr ( $secbb ) . chr ( $secbc ) . chr ( $secbd );
		$ntppacket .= chr ( $fracba ) . chr ( $fracbb ) . chr ( $fracbc ) . chr ( $fracbd );
	...
	protected function _extract($binary) {
		/*
		 * Leap Indicator bit 1100 0000 Code warning of impending leap-second to be inserted at the end of the last day of the current month.
		 */
		...
		/*
		 * Version Number bit 0011 1000 This should be 3 (RFC 1305)
		 */
		$this->_info ['version'] = ($binary ['flags'] & 0x38) >> 3;
		
		/*
		 * Mode bit 0000 0111 Except in broadcast mode, an NTP association is formed when two peers exchange messages and one or both of them create and maintain an instantiation of the protocol machine, called an association.
		 */
		...
		/*
		 * Reference Clock Identifier Identifies the particular reference clock.
		 */
		...
		/*
		 * Stratum Indicates the stratum level of the local clock
		 */
		switch ($binary ['stratum']) {
			case 0 :
		...
		/*
		 * Indicates the total roundtrip delay to the primary reference source at the root of the synchronization subnet, in seconds. Both positive and negative values, depending on clock precision and skew, are possible.
		 */
		$this->_info ['rootdelay'] = $binary ['rootdelay'];
		
		/*
		 * Indicates the maximum error relative to the primary reference source at the root of the synchronization subnet, in seconds. Only positive values greater than zero are possible.
		 */
		$this->_info ['rootdispersion'] = $binary ['rootdispersion'];
		
		/*
		 * The roundtrip delay of the peer clock relative to the local clock over the network path between them, in seconds. Note that this variable can take on both positive and negative values, depending on clock precision and skew-error accumulation.
		 */
