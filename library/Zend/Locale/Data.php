...
class Zend_Locale_Data {
	...
	public static function getList($locale, $path, $value = false) {
		...
		$val = urlencode ( $val );
		$id = strtr ( 'Zend_LocaleL_' . $locale . '_' . $path . '_' . $val, array (
				'-' => '_',
				'%' => '_',
				'+' => '_' 
		) );
		...
		if (isset ( self::$_cache )) {
			if (self::$_cacheTags) {
				self::$_cache->save ( serialize ( $temp ), $id, array (
						'Zend_Locale' 
				) );
			} else {
	...
	public static function getContent($locale, $path, $value = false) {
		...
		$val = urlencode ( $val );
		$id = strtr ( 'Zend_LocaleC_' . $locale . '_' . $path . '_' . $val, array (
				'-' => '_',
				'%' => '_',
				'+' => '_' 
		) );
	...
	public static function clearCache() {
		if (self::$_cacheTags) {
			self::$_cache->clean ( Zend_Cache::CLEANING_MODE_MATCHING_TAG, array (
					'Zend_Locale' 
			) );
		} else {
