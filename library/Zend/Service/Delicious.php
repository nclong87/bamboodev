...
class Zend_Service_Delicious {
	...
	public function __construct($uname = null, $pass = null) {
		...
		$this->_rest->getHttpClient ()->setConfig ( array (
				'ssltransport' => 'ssl' 
		) );
	...
	public function renameTag($old, $new) {
		$response = $this->makeRequest ( self::PATH_TAG_RENAME, array (
				'old' => $old,
				'new' => $new 
		) );
	...
	public function addBundle($bundle, array $tags) {
		...
		$response = $this->makeRequest ( self::PATH_BUNDLE_ADD, array (
				'bundle' => $bundle,
				'tags' => $tags 
		) );
	...
	public function deleteBundle($bundle) {
		$response = $this->makeRequest ( self::PATH_BUNDLE_DELETE, array (
				'bundle' => $bundle 
		) );
	...
	public function deletePost($url) {
		$response = $this->makeRequest ( self::PATH_POST_DELETE, array (
				'url' => $url 
		) );
	...
	public function createNewPost($title, $url) {
		return new Zend_Service_Delicious_Post ( $this, array (
				'title' => $title,
				'url' => $url 
		) );
	}
