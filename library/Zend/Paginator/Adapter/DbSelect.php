...
class Zend_Paginator_Adapter_DbSelect implements Zend_Paginator_Adapter_Interface {
	 ...
	 * query.
	 * Without setting this, {@link getPages()} selects the count
	 ...
	 *
	 * @param Zend_Db_Select|integer $totalRowCount
	...
	public function count() {
		if ($this->_rowCount === null) {
			$this->setRowCount ( $this->getCountSelect () );
		}
	...
	public function getCountSelect() {
		/**
		 * We only need to generate a COUNT query once.
		 * It will not change for
		 * this instance.
		 */
		...
		if (! empty ( $unionParts )) {
			...
			$rowCount->reset ( Zend_Db_Select::COLUMNS )->reset ( Zend_Db_Select::ORDER )->reset ( Zend_Db_Select::LIMIT_OFFSET )->reset ( Zend_Db_Select::GROUP )->reset ( Zend_Db_Select::DISTINCT )->reset ( Zend_Db_Select::HAVING )->columns ( $expression );
		}
