<?php
class Core_Controller_Helper_Utils {
	...
	public static function saveStatic($variable, $value) {
		...
		if (($static = $cache->load ( 'static' )) == null)
			$static = array ();
	...
	/*
	 * public static function getDirectories($path,$folder_name) { $dirs = scandir($path.DS.$folder_name); if($path=='.') $str = '<ul id="browser" class="filetree">'; else $str = '<ul>'; $flag = false; foreach ($dirs as $dir) { if ($dir === '.' or $dir === '..') continue; if(is_dir($path.DS.$folder_name.DS.$dir)) { $flag = true; $str.= '<li><span class="folder">'.$dir.'</span>'.zendcms_Controller_Helper_Utils::getDirectories($path.DS.$folder_name, $dir).'</li>'; } } if($flag == false) return ''; return $str.'</ul>'; }
	 */
