...
class Core_Email {
	...
	var $_mail;
	public function __construct($sec = false) {
		if ($sec == false) {
			...
			$config = array (
					'auth' => 'login',
					'username' => SENDER_EMAIL_SEC,
					'password' => SENDER_EMAIL_PASSWORD_SEC,
					'port' => SENDER_EMAIL_PORT_SEC,
					'ssl' => 'tls' 
			);
			$transport = new Zend_Mail_Transport_Smtp ( SENDER_EMAIL_SMTP_SEC, $config );
			Zend_Mail::setDefaultTransport ( $transport );
			Zend_Mail::setDefaultFrom ( SENDER_EMAIL_SEC, SENDER_EMAIL_FROM );
			Zend_Mail::setDefaultReplyTo ( REPLY_TO_EMAIL, SENDER_EMAIL_FROM );
		}
